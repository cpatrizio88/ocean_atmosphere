...
#sst_MM = sst.subRegion(latitude=(MMlatbounds[0], MMlatbounds[1]), longitude=(MMlonbounds[0], MMlatbounds[1]))
#
#MMlats = sst_MM.getLatitude()[:]
#
#MM = spatial_ave(sst_MM, MMlats)
...
tropsst = spatial_ave(sst[:,eqi:MMlatboundmaxi,MMlonboundmini:MMlonboundmaxi], lats[eqi:MMlatboundmaxi])
...
#print 'fraction of tropical SST variance accounted for by non-ENSO tropical SST variability', np.var(spatial_ave(sst_MMNH, lats[eqi:MMlatboundmaxi]))/np.var(tropsst)
...
MMstd_dv = scaler.fit_transform(MM_dv.reshape(-1,1))
...
for i in range(nlat):   
     ...
     #for j in range(nlon):
        ...
        clf = linear_model.LinearRegression()
        clf.fit(MMstd.reshape(-1,1), uprime_g)
        ucorrs[i,:] = np.squeeze(clf.coef_)
     
        clf = linear_model.LinearRegression()
        clf.fit(MMstd_lt.reshape(-1,1), u_lt_g)
        ucorrs_lt[i,:] = np.squeeze(clf.coef_)
     
        clf = linear_model.LinearRegression()
        clf.fit(MMstd_st.reshape(-1,1), u_st_g)
        ucorrs_st[i,:] = np.squeeze(clf.coef_)
     
        clf = linear_model.LinearRegression()
        clf.fit(MMstd.reshape(-1,1), vprime_g)
        vcorrs[i,:] = np.squeeze(clf.coef_)
         
        clf = linear_model.LinearRegression()
        clf.fit(MMstd_lt.reshape(-1,1), v_lt_g)
        vcorrs_lt[i,:] = np.squeeze(clf.coef_)
     
        clf = linear_model.LinearRegression()
        clf.fit(MMstd_st.reshape(-1,1), v_st_g)
        vcorrs_st[i,:] = np.squeeze(clf.coef_)
     
        clf = linear_model.LinearRegression()
        clf.fit(MMstd.reshape(-1,1), psprime_g)
        pscorrs[i,:] = np.squeeze(clf.coef_)
     
        clf = linear_model.LinearRegression()
        clf.fit(MMstd_lt.reshape(-1,1), ps_lt_g)
        pscorrs_lt[i,:] = np.squeeze(clf.coef_)
     
        clf = linear_model.LinearRegression()
        clf.fit(MMstd_st.reshape(-1,1), ps_st_g)
...
ax=plt.gcf().gca()
ax.plot(tyears, tropsst)
ax.plot(tyears[ci:-ci], running_mean(tropsst, N_map), label='{:1.0f}-yr RM'.format((N_map-1)/12.))
...
ax.set_title(r'Tropical Atlantic SST')
...
NAtropsst = spatial_ave(sst_MMNH, MMlatsNH)

SAtropsst = spatial_ave(sst_MMSH, MMlatsSH)

nonENSOtropsst = (NAtropsst + SAtropsst)/2.

ci = (N_map-1)/2
fig=plt.figure(figsize=(14,22))
ax = fig.add_subplot(311)
ax=plt.gcf().gca()
ax.plot(tyears, nonENSOtropsst)
ax.plot(tyears[ci:-ci], running_mean(nonENSOtropsst, N_map), label='{:1.0f}-yr RM'.format((N_map-1)/12.))
#plt.plot(tyears[ci:-ci], MM_st, label='short-term residual')
#ax.fill_between(tyears[ci:-ci], 0, MM_smooth, where= MM_smooth>0, color='red')
#ax.fill_between(tyears[ci:-ci], 0, MM_smooth, where= MM_smooth<0, color='blue')
ax.set_title(r'non-ENSO Tropical Atlantic SST')
ax.axhline(0, color='black')
ax.set_ylabel(r'SST ($^{{\circ}}$C)')
ax.legend(loc='upper right')
ax.set_xlabel('time (years)')
...
ax = fig.add_subplot(312)
ax=plt.gcf().gca()
ax.plot(tyears, NAtropsst)
ax.plot(tyears[ci:-ci], running_mean(NAtropsst, N_map), label='{:1.0f}-yr RM'.format((N_map-1)/12.))
#plt.plot(tyears[ci:-ci], MM_st, label='short-term residual')
#ax.fill_between(tyears[ci:-ci], 0, MM_smooth, where= MM_smooth>0, color='red')
#ax.fill_between(tyears[ci:-ci], 0, MM_smooth, where= MM_smooth<0, color='blue')
ax.set_title(r'non-ENSO NA SST')
ax.axhline(0, color='black')
ax.set_ylabel(r'SST ($^{{\circ}}$C)')
ax.set_xlabel('time (years)')
ax.legend(loc='upper right')
...
#plt.figure()
#ax=plt.gcf().gca()
ax.plot(tyears, SAtropsst)
ax.plot(tyears[ci:-ci], running_mean(SAtropsst, N_map), label='{:1.0f}-yr RM'.format((N_map-1)/12.))
ax.legend(loc='upper right')
#plt.plot(tyears[ci:-ci], MM_st, label='short-term residual')
#ax.fill_between(tyears[ci:-ci], 0, MM_smooth, where= MM_smooth>0, color='red')
#ax.fill_between(tyears[ci:-ci], 0, MM_smooth, where= MM_smooth<0, color='blue')
ax.set_title(r'non-ENSO SA SST')
ax.set_ylim(-0.6,0.6)
ax.axhline(0, color='black')
ax.set_ylabel(r'{:s} ({:s})'.format(ftitle, units))
#ax.legend()
ax.set_xlabel('time (years)')
plt.savefig(fout + '{:s}_tropSST_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname,latbounds[0], latbounds[1], str(detr)[0]))
plt.close()
...
ax=plt.gcf().gca()
ax.plot(tyears, CTI)
ax.plot(tyears[ci:-ci], running_mean(CTI, N_map), label='{:1.0f}-yr RM'.format((N_map-1)/12.))
ax.set_ylim(-0.8,0.8)
#plt.plot(tyears[ci:-ci], MM_st, label='short-term residual')
#ax.fill_between(tyears[ci:-ci], 0, MM_smooth, where= MM_smooth>0, color='red')
#ax.fill_between(tyears[ci:-ci], 0, MM_smooth, where= MM_smooth<0, color='blue')
ax.set_title(r'Cold-tongue Index (CTI)')
ax.axhline(0, color='black')
ax.set_ylabel(r'SST ($^{{\circ}}$C)')
ax.legend(loc='upper right')
ax.set_xlabel('time (years)')
plt.savefig(fout + '{:s}_CTI_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, latbounds[0], latbounds[1], str(detr)[0]))
plt.close()
#plt.figure()
...
#ax.fill_between(tyears[ci:-ci], 0, MM_smooth, where= MM_smooth>0, color='red')
#ax.fill_between(tyears[ci:-ci], 0, MM_smooth, where= MM_smooth<0, color='blue')
ax.set_title(r'Atlantic Meridional Mode (simple index)')
ax.axhline(0, color='black')
#ax.set_ylabel(r'SST ($^{{\circ}}$C)')
ax.set_xlabel('time (years)')
ax.legend(loc='upper right')
...
#plt.figure()
#ax=plt.gcf().gca()
ax.plot(tyears, MMstd_dv)
ax.set_title(r'Atlantic Meridional Mode (MCA index)')
ax.plot(tyears[ci:-ci], running_mean(MMstd_dv, N_map), label='{:1.0f}-yr RM'.format((N_map-1)/12.))
ax.legend(loc='upper right')
#plt.plot(tyears[ci:-ci], MM_st, label='short-term residual')
#ax.fill_between(tyears[ci:-ci], 0, MM_smooth, where= MM_smooth>0, color='red')
#ax.fill_between(tyears[ci:-ci], 0, MM_smooth, where= MM_smooth<0, color='blue')
...
#ax.legend()
ax.set_xlabel('time (years)')
plt.savefig(fout + '{:s}_MMdv_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, latbounds[0], latbounds[1], str(detr)[0]))
plt.close()
