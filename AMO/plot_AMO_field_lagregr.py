...
for latbounds in latboundar:
    ...
    for i in range(nlat):         
     ...
     clf = linear_model.LinearRegression()
     if ltlag > 0:
         clf.fit(AMOstd[ltlag:].reshape(-1,1), fieldprime_g[:-ltlag,:])
     else:
          clf.fit(AMOstd[:ltlag].reshape(-1,1), fieldprime_g[-ltlag:,:])
     ...
     clf = linear_model.LinearRegression()
     if ltlag > 0:
         clf.fit(AMOstd_lt[ltlag:].reshape(-1,1), field_lt_g[:-ltlag,:])
     else:
         clf.fit(AMOstd_lt[:ltlag].reshape(-1,1), field_lt_g[-ltlag:,:])
    ...
    ax = fig.add_subplot(313)
    ax.plot(tyears, NAfield2)
    ax.plot(tyears[ci:-ci], NAfield_smooth)
    ax.set_ylabel(r'{:s}  ({:s})'.format(ftitle, units))
...
#    ax.set_xlabel('{:s} lag (years)'.format(ftitle))
    ...
    plt.savefig(fout + '{:s}_AMO_{:s}_{:1.0f}lagcorr_map_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, fsave, -ltlag, latbounds[0], latbounds[1], str(detr)[0]))
    plt.close()
    ...
    plt.savefig(fout + '{:s}_AMO_{:s}_{:1.0f}lagltcorr_map_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, fsave, -ltlag, latbounds[0], latbounds[1], str(detr)[0]))
    plt.close()
...
plt.title(r'regression of {:s} on AMO ({:1.0f}$^{{\circ}}$N to {:2.0f}$^{{\circ}}$N)'.format(ftitle, latbounds[0], latbounds[1]))
plt.savefig(fout + '{:s}_AMO_{:s}_{:1.0f}lagcorr_zonalave_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, fsave, -ltlag, latbounds[0], latbounds[1], str(detr)[0]))
plt.close()

fieldcorrs_zonalave = np.ma.average(fieldcorrs_lt[:,NAminloni:NAmaxloni], axis=-1)
fig = plt.figure(figsize=(12,8))
ax = fig.gca()
ax.plot(fieldcorrs_zonalave, lats)
ax.axvline(0, color='k')
...
#cb.set_ticks(np.arange(fieldmin,fieldmax+cbstep,cbstep))
#cb.set_ticklabels(np.arange(fieldmin,fieldmax+cbstep,cbstep))
plt.title(r'long-term regression of {:s} on AMO ({:1.0f}$^{{\circ}}$N to {:2.0f}$^{{\circ}}$N)'.format(ftitle, latbounds[0], latbounds[1]))
plt.savefig(fout + '{:s}_AMO_{:s}_{:1.0f}lagltcorr_zonalave_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, fsave, -ltlag, latbounds[0], latbounds[1], str(detr)[0]))
plt.close()
