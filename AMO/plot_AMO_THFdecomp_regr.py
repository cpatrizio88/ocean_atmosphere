...
from ocean_atmosphere.misc_fns import an_ave, spatial_ave, calc_AMO, running_mean, calc_NA_globeanom, detrend_separate, detrend_common, calcsatspechum
from thermolib.thermo import q_star
...
#cD = fcD('CN')
#cD = cD.subRegion(latitude=(minlat, maxlat), longitude=(minlon, maxlon))
#cD= cD[tskip:nt_ps,:]
...
#cEH = fcE('CDH')
#cEH = cEH.subRegion(latitude=(minlat, maxlat), longitude=(minlon, maxlon))
#cEH = cEH[tskip:nt_ps,:]
...
#field=qvdiff
#fsave = 'LHFdecomp_qvdiff'
#ftitle=r'LHF$^{\prime}_{q_v}$'
#units = 'W m$^{-2}$'

#field=cE
#fsave = 'LHFdecomp_cE'
#ftitle = r'LHF$^{\prime}_{c_E}$'
#units = 'W m$^{-2}$'

#field=qvdiff
#fsave = 'qvdiff'
#ftitle=r'$\Delta q_{v,sea-air}}$'
#units = 'g/kg'

#field2= sst 
#fsave = 'LHFdecomp_qvdiffCC'
#ftitle=r'LHF$^{\prime}_{q_v, CC}$'
#units = 'W m$^{-2}$'

#field = Hdiff
#fsave = 'THFdecomp_Hdiff'
#ftitle =r'THF$^{\prime}_{H}$'
#units = 'W m$^{-2}$'

field = cE
fsave = 'THFdecomp_cE'
ftitle =r'THF$^{\prime}_{c_E}$'
units = 'W m$^{-2}$'
...
#field = umag
#ftitle = r'THF$_{|\mathbf{u}_{10m}|}$'
#fsave = 'THFdecomp_umag'
#units = 'W m$^{-2}$'
...
#qsat = q_star(ps*1e2, sst)

#field = qvsurf - field
#qvdiff = qvsurf - qv10m
#field=field

#umag = umag[:ps.shape[0],...]
#ustar = ustar[:ps.shape[0],...]
#qstar = qstar[:ps.shape[0],...]

#cE = (ustar*qstar)/(umag*field)
...
#    ax.set_xlabel('{:s} lag (years)'.format(ftitle))
    ...
    ax.set_ylabel(r'{:s}'.format(fieldunitslag))
    if corr:
        ax.set_title('long-term correlation of AMO with NA {:s} ({:1.0f}-yr RM)'.format(ftitle, windows[i]))
    else:
        ax.set_title('long-term regression of NA {:s} on AMO ({:1.0f}-yr RM)'.format(ftitle, windows[i]))
    ax.set_xlabel('{:s} lag (years)'.format(ftitle))
    ...
    plot = ax.contourf(x, y, SAVE + np.multiply(meanstate, fieldcorrs), cmap=plt.cm.RdBu_r, levels=fieldlevels, extend='both', transform=prj)
    ...
    plt.savefig(fout + '{:s}_AMO_SUMTHFdecomp_corr_map_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, latbounds[0], latbounds[1], str(detr)[0]))
    plt.close()
    ...
    plot = ax.contourf(x, y, np.multiply(meanstate, fieldcorrs_lt), cmap=plt.cm.RdBu_r, levels=fieldlevels, extend='both', transform=prj)
    cb = plt.colorbar(plot, label=r'{:s}'.format(units))
    cb.set_ticks(ticks)
    cb.set_ticklabels(ticklbls)
    plt.title(r'regression of long-term {:s} on AMO ({:1.0f}-yr RM)'.format(ftitle, N_map))
    ...
    plot = ax.contourf(x, y, np.multiply(meanstate, fieldcorrs_st), cmap=plt.cm.RdBu_r, levels=fieldlevels, extend='both', transform=prj)
    cb = plt.colorbar(plot, label=r'{:s}'.format(units))
    cb.set_ticks(ticks)
    cb.set_ticklabels(ticklbls)
    plt.title(r'regression of short-term {:s} on AMO ({:1.0f}-yr RM residual)'.format(ftitle, N_map))
...
if fsave == 'THFdecomp_qvdiff' or fsave == 'THFdecomp_cE' or fsave == 'SHFdecomp_cE' or fsave == 'SHFdecomp_tdiff' or fsave == 'THFdecomp_cE' or fsave == 'THFdecomp_Hdiff':
   fieldcorrs_zonalave = np.ma.average(meanstate[:,NAminloni:NAmaxloni]*fieldcorrs[:,NAminloni:NAmaxloni], axis=-1)
else:
...
ax.axvline(0, color='k')
if corr:
    ax.set_title('correlation of NA {:s} with AMO'.format(ftitle))
else:
    ax.set_title('regression of NA {:s} on AMO'.format(ftitle))
