...
#        ax = fig.add_subplot(221)
#        ax.plot(lags, NAfieldlagcorrs_lt[i,:], label='{:s}'.format(ftitle))
#        ax.axhline(0, color='black')
#        ax.axvline(0, color='black')
#        ax.set_ylim(fieldminlag,fieldmaxlag)
#        ax.set_ylabel(r'{:s}'.format(fieldunitslag))
#        if corr:
#            ax.set_title('long-term correlation ({:1.0f}-yr RM)'.format(windows[i]))
#        else:
#            ax.set_title('long-term regression  ({:1.0f}-yr RM)'.format(windows[i]))
...
#            plt.legend()
#        if corr:
#            plt.savefig(fout + '{:s}_AMO_surfenergy_lagcorr_smoothhist_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, latbounds[0], latbounds[1], str(detr)[0]))
#        else:
#            plt.savefig(fout + '{:s}_AMO_surfenergy_lagregr_smoothhist_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, latbounds[0], latbounds[1], str(detr)[0]))
...
#        plt.savefig(fout + '{:s}_AMO_{:s}_stcorr_map_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, fsave, latbounds[0], latbounds[1], str(detr)[0]))
        ...
        fig = plt.figure(2+latcount, figsize=(12,8))
        ax = fig.gca()
        ax.plot(fieldcorrs_zonalave, lats, label='{:s}'.format(ftitle))
        ax.axvline(0, color='k')
        ax.get_yaxis().set_tick_params(direction='out')
        ax.get_xaxis().set_tick_params(direction='out')
        ...
        plt.title(r'regression on AMO ({:1.0f}$^{{\circ}}$N to {:2.0f}$^{{\circ}}$N)'.format(latbounds[0], latbounds[1]))
        if counter == numfields-1:
            plt.legend()
        plt.savefig(fout + '{:s}_AMO_surfenergy_{:1.0f}lagcorr_zonalave_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, -ltlag, latbounds[0], latbounds[1], str(detr)[0]))
        if counter == numfields-1:
            plt.close()
        #plt.close()
#    
