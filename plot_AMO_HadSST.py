...
from AMO.misc_fns import an_ave, spatial_ave, running_mean, calc_AMO
...
nlon = sst_globe.shape[2]
nlat = sst_globe.shape[1]
nt = AMO.shape[0]

lats = sst_globe.getLatitude()[:]
lons = sst_globe.getLongitude()[:]
...
for i in range(nlat):
    for j in range(nlon):
         ...
         slope, intercept, r_value, p_value, std_err = linregress(AMO_std, sst_globe_an[:,i,j])
...
par = np.arange(-90.,90,15.)
mer = np.arange(0,360,30.)
...
plt.figure(figsize=(6,4))
#m = Basemap(projection='cyl',llcrnrlat=0,urcrnrlat=75,llcrnrlon=-80,urcrnrlon=0,resolution='i')
...
AMO = AMO.getValue()
sstanom_anave = sstanom_anave.getValue()
nasstanom_anave = nasstanom_anave.getValue()
...
plt.savefig(fout + 'HadSST1_globesst.pdf')
plt.close()

#nasstanom_an = an_ave(nasstanom_ave).getValue()


plt.figure()
plt.plot(tplot, nasstanom_anave)
...
plt.title('NA SST anomaly')
plt.ylim(-1,1)
plt.xlabel('time (years)')
plt.ylabel(r'SST ($^{{\circ}}$C)')
plt.axhline(0, color='black')
plt.savefig(fout + 'HadSST1_nasst.pdf')
plt.close()
...
plt.title('AMO, 11-year smoothing')
plt.xlabel('time (years)')
plt.ylabel(r'SST ($^{{\circ}}$C)')
plt.ylim(-1,1)
plt.savefig(fout + 'HadSST1_AMO_{:3.0f}yearsmooth.pdf'.format(N))
plt.close()
