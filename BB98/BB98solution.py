...
fradtoa = xr.open_dataset(fin + 'MERRA2_TOArad_monthly1980to2017.nc')
...
# Calculate: 
# - power of T_o, surface heat flux forcing and ocean dynamics forcing 
# - regression of turbulent heat fluxes on T_o - T_a (averaged over spatial region)
# - climatological T_o and T_a (averaged over spatial region)
# - ocean dynamics damping (averaged over spatial region)
# - variance of surface heat flux forcing and ocean dynamics forcing (averaged over spatial region)

#lambda_sa = lmbda + 4*eps_a*sig_b*sstbar^3  
#lambda_so = lmbda + 4*eps_a*sig_b*tabar^3  
#lambda_a = 4*eps_a*sig_b*(2*tabar^3 - sstbar^3)
#lambda_o = 4*sig_b*(sstbar^3 - eps_a*tabar^3) + lambda_odyn
...
lmbda_ave =  st2.spatial_ave_xr(lmbda.sel(lon=slice(lonbounds_ave[0],lonbounds_ave[1])), lats=lats.sel(lat=slice(latbounds_ave[0],latbounds_ave[1])))
lambda_sa_ave =  st2.spatial_ave_xr(lambda_sa.sel(lon=slice(lonbounds_ave[0],lonbounds_ave[1])), lats=lats.sel(lat=slice(latbounds_ave[0],latbounds_ave[1])))
lambda_so_ave =  st2.spatial_ave_xr(lambda_so.sel(lon=slice(lonbounds_ave[0],lonbounds_ave[1])), lats=lats.sel(lat=slice(latbounds_ave[0],latbounds_ave[1])))

lambda_a_ave =  st2.spatial_ave_xr(lambda_a.sel(lon=slice(lonbounds_ave[0],lonbounds_ave[1])), lats=lats.sel(lat=slice(latbounds_ave[0],latbounds_ave[1])))
lambda_o_ave =  st2.spatial_ave_xr(lambda_o.sel(lon=slice(lonbounds_ave[0],lonbounds_ave[1])), lats=lats.sel(lat=slice(latbounds_ave[0],latbounds_ave[1])))
...
lambdaQ_s_ave =  st2.spatial_ave_xr(lambdaQ_s.sel(lon=slice(lonbounds_ave[0],lonbounds_ave[1])), lats=lats.sel(lat=slice(latbounds_ave[0],latbounds_ave[1])))

sstbar_ave =  st2.spatial_ave_xr(sstbar.sel(lon=slice(lonbounds_ave[0],lonbounds_ave[1])), lats=lats.sel(lat=slice(latbounds_ave[0],latbounds_ave[1])))
tabar_ave =  st2.spatial_ave_xr(tabar.sel(lon=slice(lonbounds_ave[0],lonbounds_ave[1])), lats=lats.sel(lat=slice(latbounds_ave[0],latbounds_ave[1])))
...
Qopower_ave = st2.spatial_ave_xr(Qrpower.sel(lon=slice(lonbounds_ave[0],lonbounds_ave[1])), lats=lats.sel(lat=slice(latbounds_ave[0],latbounds_ave[1])))
Qspower_ave = st2.spatial_ave_xr(Qspower.sel(lon=slice(lonbounds_ave[0],lonbounds_ave[1])), lats=lats.sel(lat=slice(latbounds_ave[0],latbounds_ave[1])))
...
QsQopower_ave = st2.spatial_ave_xr(QsQrpower.sel(lon=slice(lonbounds_ave[0],lonbounds_ave[1])), lats=lats.sel(lat=slice(latbounds_ave[0],latbounds_ave[1])))

FaFopower_ave = st2.spatial_ave_xr(QsQrstarpower.sel(lon=slice(lonbounds_ave[0],lonbounds_ave[1])), lats=lats.sel(lat=slice(latbounds_ave[0],latbounds_ave[1])))

covQsQr_ave = st2.spatial_ave_xr(covQsQr.sel(lon=slice(lonbounds_ave[0],lonbounds_ave[1])), lats=lats.sel(lat=slice(latbounds_ave[0],latbounds_ave[1])))
covQsQrstar_ave = st2.spatial_ave_xr(covQsQrstar.sel(lon=slice(lonbounds_ave[0],lonbounds_ave[1])), lats=lats.sel(lat=slice(latbounds_ave[0],latbounds_ave[1])))

Q_s_var_ave = st2.spatial_ave_xr(Q_s_var.sel(lon=slice(lonbounds_ave[0],lonbounds_ave[1])), lats=lats.sel(lat=slice(latbounds_ave[0],latbounds_ave[1])))
Q_o_var_ave = st2.spatial_ave_xr(Q_o_var.sel(lon=slice(lonbounds_ave[0],lonbounds_ave[1])), lats=lats.sel(lat=slice(latbounds_ave[0],latbounds_ave[1])))

F_a_power_ave_sum = F_a_power_ave.sum('freq')
F_o_power_ave_sum = F_o_power_ave.sum('freq')
Qspower_ave_sum = Qspower_ave.sum('freq')
Qopower_ave_sum = Qopower_ave.sum('freq')

#print('Tpower_ave', Tpower_ave)
#print('lambda_ave', lmbda_ave)
#print('lambda_sa_ave', lambda_sa_ave)
#print('lambda_so_ave', lambda_so_ave)
#print('lambda_a_ave', lambda_a_ave)
#print('lambda_o_ave', lambda_o_ave)
print('F_a_var_ave', F_a_var_ave)
print('F_o_var_ave', F_o_var_ave)
print('Q_s_var_ave', Q_s_var_ave)
print('Q_o_var_ave', Q_o_var_ave)
#print('covQsQr_ave', covQsQr_ave)
#print('covQsQrstar_ave', covQsQrstar_ave)

print ('F_a_power_ave_sum', F_a_power_ave_sum)
print ('F_o_power_ave_sum', F_o_power_ave_sum)
print ('Qspower_ave_sum', Qspower_ave_sum)
print ('Qopower_ave_sum', Qopower_ave_sum)

#print('F_a_std_ave', np.sqrt(F_a_var_ave))
#print('F_o_std_ave', np.sqrt(F_o_var_ave))

print('sstbar_ave', sstbar_ave)
print('tabar_ave', tabar_ave)
...
sigma_o = gamma_o*2*np.pi*1j*freq_sec + lambda_s + lambda_o
sigma_a = gamma_a*2*np.pi*1j*freq_sec + lambda_s + lambda_a

# White-noise assumption
F_a_pow_an = F_a_var_ave/len(f)
F_o_pow_an = F_o_var_ave/len(f)

F_aF_o_pow_an = covQsQrstar_ave/len(f)

# # Prescribe observed forcing variance
#F_a_pow_an = F_a_power_ave
#F_o_pow_an = F_o_power_ave

coef = np.abs(sigma_o*sigma_a - lambda_s**2)**2

# Compute the contributions 
#T_o_Fa = (lambda_s**2*F_a_pow_an + lambda_s*np.abs(sigma_a)*F_aF_o_pow_an)/coef
#T_o_Fo = (np.abs(sigma_a)**2*F_o_pow_an + lambda_s*np.abs(sigma_a)*F_aF_o_pow_an)/coef

T_o_Fa = (lambda_s**2*F_a_pow_an)/coef
T_o_Fo = (np.abs(sigma_a)**2*F_o_pow_an)/coef
...
#T_o_power_an = (lambda_s_ave**2*F_a_pow_an + np.abs(sigma_a)**2*F_o_pow_an + 2*lambda_s_ave*np.abs(sigma_a)*FaFopower_ave)/coef


#print('lambdaQ_s_ave', -lambdaQ_s_ave)
# Plotting
...
plt.figure(figsize=(10,12))
plt.plot(f,T_o_power_an_ave,label='$T_{o,soln}$',color='k')
plt.plot(f,Tpower_ave,label='$T_{o,obs}$',alpha=0.6,color='grey')
plt.plot(f,T_o_Fa_ave,label='$\widetilde{F}_a$',color='g')
plt.plot(f,T_o_Fo_ave, label='$\widetilde{F}_o$', color='b')
plt.legend(loc='best')
ax=plt.gca()
ax.set_xscale('log')
ax.set_yscale('log')
...
ax.yaxis.set_minor_locator(locmin)
ax.yaxis.set_minor_formatter(matplotlib.ticker.NullFormatter())
plt.savefig(fout + '{:s}_forcingcont_powerspectra_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, latbounds_ave[0], latbounds_ave[1], str(detr)[0]))

plt.figure(figsize=(10,12))
plt.plot(f,T_o_Fo_ave/T_o_Fa_ave, color='k')
#plt.legend(loc='best')
ax=plt.gca()
ax.set_xscale('log')
#ax.set_yscale('log')
...
ax.yaxis.set_minor_locator(locmin)
ax.yaxis.set_minor_formatter(matplotlib.ticker.NullFormatter())
plt.savefig(fout + '{:s}_forcingratio_powerspectra_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, latbounds_ave[0], latbounds_ave[1], str(detr)[0]))
...
mapper = Mapper()
m,ax=mapper(T_o_var_sum, log=True, bnds=bnds, title='$\sigma_T^2$', units=r'K$^{2}$',  cbfrac=cbfrac, cmap=sstcmap, vmin=0.01, vmax=sstvmax)
if drawbox:
    ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1,linestyle='dashed',transform=cart.crs.PlateCarree()))
...
mapper = Mapper()
m,ax=mapper(Q_s_var, bnds=bnds, logscale=False, log=True,  title=r'$\overline{Q^{\,\prime 2}_s}$', units=r'W$^2$ m$^{-4}$', cbfrac=cbfrac, cmap=sstcmap, vmin=varmin, vmax=varmax)
if drawbox:
    ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1, linestyle='dashed', transform=cart.crs.PlateCarree()))
...
mapper = Mapper()
m,ax=mapper(Q_o_var, bnds=bnds, logscale=False, log=True, title=r'$\overline{Q^{\,\prime 2}_o}$',  units=r'W$^2$ m$^{-4}$', cbfrac=cbfrac, cmap=sstcmap, vmin=varmin, vmax=varmax)
if drawbox:
    ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1, linestyle='dashed', transform=cart.crs.PlateCarree()))
...
mapper = Mapper()
mapper(covQsQr, bnds=bnds, logscale=False, norm=lognorm, title=r'$\overline{Q_s^{\prime}Q_o^{\prime}}$', units=r'W$^2$ m$^{-4}$', cbfrac=cbfrac, cmap=cc.cm.CET_L17_r, vmin=-varmax, vmax=-varmin)
...
mapper = Mapper()
mapper(covQsQrstar, bnds=bnds, logscale=False, norm=lognorm, title='$\overline{F_a^{\prime}F_o^{\prime}}$', units=r'W$^2$ m$^{-4}$', cbfrac=cbfrac, cmap=cc.cm.CET_L17_r, vmin=-varmax, vmax=-varmin)
plt.savefig(fout + '{:s}_covFaFo_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()
