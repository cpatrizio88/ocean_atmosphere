...
F_oF_a_power_ave = st2.spatial_ave_xr(QsQrstarpower.sel(lon=slice(lonbounds_ave[0],lonbounds_ave[1])), lats=lats.sel(lat=slice(latbounds_ave[0],latbounds_ave[1])))
...
F_oF_a_var = QsQrstarpower.sum('freq')
...
# F_a_pow_an = (F_a_var_ave+covQsQrstar)/len(f)
# F_o_pow_an = (F_o_var_ave+covQsQrstar)/len(f)

# Use observed variances 
# F_a_pow_an = F_a_power_ave + F_oF_a_power_ave
# F_o_pow_an = F_o_power_ave + F_oF_a_power_ave
...
lambda_tot_ave =  st2.spatial_ave_xr(lambda_tot.sel(lon=slice(lonbounds_ave[0],lonbounds_ave[1])), lats=lats.sel(lat=slice(latbounds_ave[0],latbounds_ave[1])))
lambda_s_ave=  st2.spatial_ave_xr(lambda_s.sel(lon=slice(lonbounds_ave[0],lonbounds_ave[1])), lats=lats.sel(lat=slice(latbounds_ave[0],latbounds_ave[1])))
...
print('lambda_tot_ave', lambda_tot_ave)
print('lambda_s_ave', lambda_s_ave)
...
mapper = Mapper()
m,ax=mapper(lambda_tot, logscale=False, bnds=bnds, title=r'$\lambda_{tot}$', units=r'W m$^{-2}$ K$^{-1}$', cbfrac=cbfrac, cmap=fieldcmap, vmin=-lambdamax, vmax=lambdamax)
if drawbox:
    ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1, linestyle='dashed', transform=cart.crs.PlateCarree()))
plt.savefig(fout + '{:s}_lambda_tot_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()

mapper = Mapper()
m,ax=mapper(lambda_s, logscale=False, bnds=bnds, title=r'$\lambda_{s}$', units=r'W m$^{-2}$ K$^{-1}$', cbfrac=cbfrac, cmap=fieldcmap, vmin=-lambdamax, vmax=lambdamax)
if drawbox:
    ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1, linestyle='dashed', transform=cart.crs.PlateCarree()))
plt.savefig(fout + '{:s}_lambda_s_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()
