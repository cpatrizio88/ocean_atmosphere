...
#field = sst
ssttitle = r'$\partial T_s / \partial t$'
sstsave = 'dSSTdt'
sstunits = 'K/s'
...
if not(corr):
    ...
    if fsave == 'sst':
        plt.title(r'autoregression of {:s}'.format(ssttitle))
    else:
        plt.title(r'Regression of {:s} on {:s}'.format(ftitle, ssttitle))
    plt.savefig(fout + '{:s}_monthlylocal{:s}_{:s}_corr_map_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, sstsave, fsave, latbounds[0], latbounds[1], str(detr)[0]))
    plt.close()
    ...
    if lterm:
        ...
        cb.set_ticks(ticks)
        cb.set_ticklabels(ticklbls)
        plt.title(r'regression of long-term {:s} on {:s} ({:1.0f}-month RM)'.format(ftitle, ssttitle, N_map-1))
        plt.savefig(fout + '{:s}_monthlylocal{:s}_{:s}_ltcorr_map_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, sstsave, fsave, latbounds[0], latbounds[1], str(detr)[0]))
        plt.close()
        ...
        cb.set_ticks(ticks)
        cb.set_ticklabels(ticklbls)
        plt.title(r'regression of short-term {:s} on {:s} (residual)'.format(ftitle, ssttitle))
        plt.savefig(fout + '{:s}_monthlylocal{:s}_{:s}_stcorr_map_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, sstsave, fsave, latbounds[0], latbounds[1], str(detr)[0]))
        plt.close()
   ...
   #UNCOMMENT TO PLOT LEAD-LAG REGRESSION MAPS
    ...
    for leadi in range(len(lags)/2):
        ...
        cb.set_ticklabels(ticklbls)
        if fsave == 'sst':
            plt.title(r'autoregression of {:s} ({:1.0f} month lag)'.format(ftitle, lags[leadi]))
        else:
            plt.title(r'{:s} ({:1.0f} month lag)'.format(ftitle, lags[leadi]))
        plt.savefig(fout + '{:s}_monthlylocal{:s}_{:s}_lag{:1.0f}corr_map_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, sstsave, fsave, lags[leadi], latbounds[0], latbounds[1], str(detr)[0]))
        plt.close()
        ...
        ax.set_extent((bnds[0], bnds[1], bnds[2], bnds[3]), crs=cart.crs.PlateCarree())
        #ax.contour(x, y, pscorrs, levels=SLPlevels, colors='k', inline=1, linewidths=1)
        plot = ax.contourf(x, y, fieldlagcorrs[lagi,...], cmap=cmap, levels=fieldlevels, extend='both', transform=cart.crs.PlateCarree())
        cb = plt.colorbar(plot, orientation = orient, label=r'{:s}'.format(units))
        cb.set_ticks(ticks)
        cb.set_ticklabels(ticklbls)
        if fsave == 'sst':
            plt.title(r'autoregression of {:s} ({:1.0f} month lag)'.format(ftitle, lags[lagi]))
        else:
            plt.title(r' {:s} ({:1.0f} month lag)'.format(ftitle, lags[lagi]))
        plt.savefig(fout + '{:s}_monthlylocal{:s}_{:s}_lag{:1.0f}corr_map_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, sstsave, fsave, lags[lagi], latbounds[0], latbounds[1], str(detr)[0]))
        plt.close()
        ...
        if lterm:
            ...
            cb.set_ticklabels(ticklbls)
            if fsave == 'sst':
                plt.title(r'autoregression of long-term {:s} ({:1.0f} month lag)'.format(ftitle, lags[leadi]))
            else:
                plt.title(r'regression of long-term {:s} on {:s} ({:1.0f} month lag)'.format(ftitle, ssttitle, lags[leadi]))
            plt.savefig(fout + '{:s}_monthlylocal{:s}_{:s}_lag{:1.0f}ltcorr_map_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, sstsave, fsave, lags[leadi], latbounds[0], latbounds[1], str(detr)[0]))
            plt.close()
            ...
            cb.set_ticklabels(ticklbls)
            if fsave == 'sst':
                plt.title(r'autoregression of short-term {:s} ({:1.0f} month lag)'.format(ftitle, lags[leadi]))
            else:
                plt.title(r'Short-term {:s} ({:1.0f} month lag)'.format(ftitle, lags[leadi]))
            plt.savefig(fout + '{:s}_monthlylocal{:s}_{:s}_lag{:1.0f}stcorr_map_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, sstsave, fsave, lags[leadi], latbounds[0], latbounds[1], str(detr)[0]))
            plt.close()
...
if lagmax > 12:
    lagg = lagg/12.
    laglabels = (laglabels/12.).astype(int)
    lagunits = 'years'
else:
    lagunits = 'months'
...
if lterm:
    ...
    ax.axvline(0, color='k')
    ax.set_title('Long-term {:s} ({:1.0f}-yr RM)'.format(ftitle, N_map-1))
    ax.set_xlabel(r'{:s} Lag ({:s})'.format(ftitle, lagunits))
    ax.set_ylim(lats[0],lats[-1])
    ax.set_xticks(laglabels)
    ax.set_xticklabels(laglabels)
    ...
    cb.set_ticklabels(ticklbls)
    if corr:
        plt.savefig(fout + '{:s}_monthlylocal{:s}_{:s}_lagcorrlagmax{:2.0f}_zonalave_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, sstsave, fsave, lagmax, latbounds[0], latbounds[1], str(detr)[0]))
    else:
        plt.savefig(fout + '{:s}_monthlylocal{:s}_{:s}_lagregrlagmax{:2.0f}_zonalave_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, sstsave, fsave, lagmax, latbounds[0], latbounds[1], str(detr)[0]))
    plt.close() 
else:
    ...
    ax.axvline(0, color='k')
    if corr:
            plt.savefig(fout + '{:s}_monthlylocal{:s}_{:s}_lagcorr_lagmax{:3.0f}_zonalave_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, sstsave, fsave, lagmax, latbounds[0], latbounds[1], str(detr)[0]))
    else:
            plt.savefig(fout + '{:s}_monthlylocal{:s}_{:s}_lagregr_lagmax{:3.0f}_zonalave_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, sstsave, fsave, lagmax, latbounds[0], latbounds[1], str(detr)[0]))
    plt.close() 
