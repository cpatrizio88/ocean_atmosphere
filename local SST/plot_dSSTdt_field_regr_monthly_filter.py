...
field_lt = np.ma.masked_array(field_lt, mask=~np.isfinite(fieldprime))
...
Asst = spatial_ave(sstprime, lats)
Asst_lt = spatial_ave(sst_lt, lats)

Afield = spatial_ave(fieldprime, lats)
Afield_lt = spatial_ave(field_lt, lats)


Asst_lt3 = butter_lowpass_filter(Asst, 1/(12*3.), fs, order)
Afield_lt3 = butter_lowpass_filter(Afield, 1/(12*3.), fs, order)

Asst_lt5 = butter_lowpass_filter(Asst, 1/(12*5.), fs, order)
Afield_lt5 = butter_lowpass_filter(Afield, 1/(12*5.), fs, order)

Asst_lt7 = butter_lowpass_filter(Asst, 1/(12*7.), fs, order)
Afield_lt7 = butter_lowpass_filter(Afield, 1/(12*7.), fs, order)
...
#plt.figure()
#ax=plt.gcf().gca()
ax.plot(tyears, Asst, linewidth=1)
ax.plot(tyears, Asst_lt3, label='3-yr LP')
#ax.plot(tyears, Asst_lt5, label='5-yr LP')
ax.plot(tyears, Asst_lt7, label='7-yr LP')
...
ax.set_title(r'{:s}'.format(ssttitle))
ax.axhline(0, color='black')
ax.set_ylabel(r'{:s} ({:s})'.format(ssttitle, sstunits))
ax.set_xlabel('time (years)')
ax.legend(loc='upper right')
ax = fig.add_subplot(212)
#plt.figure()
#ax=plt.gcf().gca()
ax.plot(tyears, Afield, linewidth=1)
ax.plot(tyears, Afield_lt3, label='3-yr LP')
#ax.plot(tyears, Afield_lt5, label='5-yr LP')
ax.plot(tyears, Afield_lt7, label='7-yr LP')
ax.legend(loc='upper right')
#plt.plot(tyears[ci:-ci], AMO_st, label='short-term residual')
#ax.fill_between(tyears[ci:-ci], 0, AMO_smooth, where= AMO_smooth>0, color='red')
#ax.fill_between(tyears[ci:-ci], 0, AMO_smooth, where= AMO_smooth<0, color='blue')
...
#ax.legend()
ax.set_xlabel('time (years)')
plt.savefig(fout + '{:s}_{:s}{:s}_monthly_filter_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, sstsave, fsave, latbounds[0], latbounds[1], str(detr)[0]))
plt.close()
...
if lterm:
    ...
    plt.savefig(fout + '{:s}_monthlylocal{:s}_{:s}_ltcorr_LP{:1.0f}_map_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, sstsave, fsave, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
    plt.close()
    ...
    plt.savefig(fout + '{:s}_monthlylocal{:s}_{:s}_stcorr_HP{:1.0f}_map_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, sstsave, fsave, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
    plt.close()
