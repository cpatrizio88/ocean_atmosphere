     ...
     #for j in range(nlon):
        ...
        for lag in lags:
                 ...
                 if lag > 0:
                    ...
                    lagcoefswarm = np.diag(np.ma.cov(sstwarmstd[:-lag,:], fieldwarmstd[lag:], rowvar=False)[:N,N:])
                    lagcoefscold = np.diag(np.ma.cov(sstcoldstd[:-lag,:], fieldcoldstd[lag:], rowvar=False)[:N,N:])
                ...
                #THF LEADS SST
                    ...
                    #M = sststd.shape[0]
                    lagcoefswarm = np.diag(np.ma.cov(sstwarmstd, fieldwarmstd, rowvar=False)[:N,N:])
                    lagcoefscold = np.diag(np.ma.cov(sstcoldstd, fieldcoldstd, rowvar=False)[:N,N:])
                 ...
                 fieldlagcorrs_warm[lag+lagmax,i,:] = lagcoefswarm
                 fieldlagcorrs_cold[lag+lagmax,i,:] = lagcoefscold
...
plot = ax[0,].contourf(x, y, fieldcorrs_cold, cmap=cmap, levels=fieldlevels, extend='both', transform=cart.crs.PlateCarree())
#ax.set_aspect('auto')
#cb = plt.colorbar(plot, orientation = 'horizontal', label=r'{:s}'.format(units))
#cb.set_ticks(ticks)
#cb.set_ticklabels(ticklbls)
if fsave == 'sst':
    ax[0,].set_title(r'autoregression of SST$_{{cold}}$')
else:
...
#ax.contour(x, y, pscorrs, levels=SLPlevels, colors='k', inline=1, linewidths=1)
plot = ax[1,].contourf(x, y, fieldcorrs_warm,  cmap=cmap, levels=fieldlevels, extend='both', transform=cart.crs.PlateCarree())
#ax.set_aspect('auto')
...
cb.set_ticklabels(ticklbls)
if fsave == 'sst':
    ax[1,].set_title(r'autoregression of SST$_{{warm}}$')
else:
...
ax[0,].set_extent((bnds[0], bnds[1], bnds[2], bnds[3]), crs=cart.crs.PlateCarree())
#ax.contour(x, y, pscorrs, levels=SLPlevels, colors='k', inline=1, linewidths=1)
plot = ax[0,].contourf(x, y, fieldlagcorrs_cold[leadi,...], cmap=cmap, levels=fieldlevels, extend='both', transform=cart.crs.PlateCarree())
#ax.set_aspect('auto')
#cb = plt.colorbar(plot, orientation = 'horizontal', label=r'{:s}'.format(units))
#cb.set_ticks(ticks)
#cb.set_ticklabels(ticklbls)
if fsave == 'sst':
    ax[0,].set_title(r'autoregression of SST$_{{cold}}$ ({:1.0f} month lag)'.format(lags[leadi]))
else:
    ax[0,].set_title(r'regression of {:s} on SST$_{{cold}}$ ({:1.0f} month lag)'.format(ftitle, lags[leadi]))
#ax = plt.subplot(2,1,2, projection=prj)
ax[1,].coastlines()
ax[1,].add_feature(cart.feature.LAND, zorder=99, edgecolor='k', facecolor='grey')
ax[1,].set_xticks(mer, crs=prj)
...
ax[1,].set_extent((bnds[0], bnds[1], bnds[2], bnds[3]), crs=cart.crs.PlateCarree())
#ax.contour(x, y, pscorrs, levels=SLPlevels, colors='k', inline=1, linewidths=1)
plot = ax[1,].contourf(x, y, fieldlagcorrs_warm[leadi,...], cmap=cmap, levels=fieldlevels, extend='both', transform=cart.crs.PlateCarree())
#ax.set_aspect('auto')
#cbar_ax = fig.add_axes([0.85, 0.15, 0.05, 0.7])
cb = fig.colorbar(plot, ax=ax.ravel().tolist(), fraction=0.046, pad=0.07, orientation = 'horizontal', label=r'{:s}'.format(units))
cb.set_ticks(ticks)
cb.set_ticklabels(ticklbls)
if fsave == 'sst':
    ax[1,].set_title(r'autoregression of ST$_{{warm}}$ ({:1.0f} month lag)'.format(lags[leadi]))
else:
    ax[1,].set_title(r'regression of {:s} on SST$_{{warm}}$ ({:1.0f} month lag)'.format(ftitle, lags[leadi]))

plt.savefig(fout + '{:s}_monthlylocalSST_seasons_{:s}_lag{:1.0f}corr_map_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, fsave, lags[leadi], latbounds[0], latbounds[1], str(detr)[0]))
plt.close()
...
ax[0,].set_extent((bnds[0], bnds[1], bnds[2], bnds[3]), crs=cart.crs.PlateCarree())
#ax.contour(x, y, pscorrs, levels=SLPlevels, colors='k', inline=1, linewidths=1)
plot = ax[0,].contourf(x, y, fieldlagcorrs_cold[lagi,...], cmap=cmap, levels=fieldlevels, extend='both', transform=cart.crs.PlateCarree())
#ax.set_aspect('auto')
#cb = plt.colorbar(plot, orientation = 'horizontal', label=r'{:s}'.format(units))
#cb.set_ticks(ticks)
#cb.set_ticklabels(ticklbls)
if fsave == 'sst':
    ax[0,].set_title(r'autoregression of SST$_{{cold}}$ ({:1.0f} month lag)'.format(lags[lagi]))
else:
    ax[0,].set_title(r'regression of {:s} on SST$_{{cold}}$ ({:1.0f} month lag)'.format(ftitle, lags[lagi]))
#ax = plt.subplot(2,1,2, projection=prj)
ax[1,].coastlines()
ax[1,].add_feature(cart.feature.LAND, zorder=99, edgecolor='k', facecolor='grey')
ax[1,].set_xticks(mer, crs=prj)
...
ax[1,].set_extent((bnds[0], bnds[1], bnds[2], bnds[3]), crs=cart.crs.PlateCarree())
#ax.contour(x, y, pscorrs, levels=SLPlevels, colors='k', inline=1, linewidths=1)
plot = ax[1,].contourf(x, y, fieldlagcorrs_warm[lagi,...], cmap=cmap, levels=fieldlevels, extend='both', transform=cart.crs.PlateCarree())
#ax.set_aspect('auto')
#cbar_ax = fig.add_axes([0.85, 0.15, 0.05, 0.7])
cb = fig.colorbar(plot, ax=ax.ravel().tolist(), fraction=0.046, pad=0.07, orientation = 'horizontal', label=r'{:s}'.format(units))
cb.set_ticks(ticks)
cb.set_ticklabels(ticklbls)
if fsave == 'sst':
    ax[1,].set_title(r'autoregression of SST$_{{warm}}$ ({:1.0f} month lag)'.format(lags[lagi]))
else:
    ax[1,].set_title(r'regression of {:s} on SST$_{{warm}}$ ({:1.0f} month lag)'.format(ftitle, lags[lagi]))
#plt.tight_layout()
plt.savefig(fout + '{:s}_monthlylocalSST_seasons_{:s}_lag{:1.0f}corr_map_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, fsave, lags[lagi], latbounds[0], latbounds[1], str(detr)[0]))
plt.close()
