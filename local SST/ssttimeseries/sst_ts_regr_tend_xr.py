...
if lowpass:
    ...
    ps = st.butter_lowpass_filter_xr(Q_ek, cutoff, fs, order)
...
labels=['$C_o\frac{\partial T}{\partial t}$', r'${Q_s}$', r'${Q_o}$']


ymin=-220
ymax=220
#plt.figure(figsize=(16,14))
#f, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(16,14),sharex=True)
#ax1.set_ylabel('Heat Flux (W/m$^{2}$)')
#ax1.set_ylim(ymin,ymax)
#ax1.set_title('$C_o\frac{\partial T}{\partial t}$')
#o1,=ax1.plot(tendsst.time, (Cbar*tendsst).sel(lon=reflon,lat=reflat),color='k')
#ax2.set_ylabel('Heat Flux (W/m$^{2}$)')
#ax2.set_title(r'${Q_s}$')
#ax2.set_ylim(ymin,ymax)
#o2,=ax2.plot(Q_s.time, Q_s.sel(lon=reflon,lat=reflat),color='C0')
#o3,=ax3.plot(Qr.time, Qr.sel(lon=reflon,lat=reflat),color='C2')
##plt.plot(sst.time, sst.sel(lon=reflon,lat=reflat), label='T', color='C5')
#ax3.set_xlabel('Time (year)')
#ax3.set_ylabel('Heat Flux (W/m$^{2}$)')
#ax3.set_title(r'${Q_o}$')
#ax3.set_ylim(ymin,ymax)
##plt.figlegend([o1,o2,o3], labels, loc = 'upper right', ncol=5, labelspacing=0.)
...
Qs_sa = st2.spatial_ave_xr(Q_s.sel(lat=slice(latbounds[0],latbounds[1]), lon=slice(lonbounds[0], lonbounds[1])), lats_ave)
Qo_sa = st2.spatial_ave_xr(Qr.sel(lat=slice(latbounds[0],latbounds[1]), lon=slice(lonbounds[0], lonbounds[1])), lats_ave)
tendsst_sa = st2.spatial_ave_xr(tendsst.sel(lat=slice(latbounds[0],latbounds[1]), lon=slice(lonbounds[0], lonbounds[1])), lats_ave)
Cbar_sa = st2.spatial_ave_xr(Cbar.sel(lat=slice(latbounds[0],latbounds[1]), lon=slice(lonbounds[0], lonbounds[1])), lats_ave)
sst_sa = st2.spatial_ave_xr(sst.sel(lat=slice(latbounds[0],latbounds[1]), lon=slice(lonbounds[0], lonbounds[1])), lats_ave)

Q_s_o_sa = st2.spatial_ave_xr(Q_s_o.sel(lat=slice(latbounds[0],latbounds[1]), lon=slice(lonbounds[0], lonbounds[1])), lats_ave)
Q_s_r_sa = st2.spatial_ave_xr(Q_s_r.sel(lat=slice(latbounds[0],latbounds[1]), lon=slice(lonbounds[0], lonbounds[1])), lats_ave)
#ps_sa = st2.spatial_ave_xr(ps,lats)
...
SLPregQo, intercept = st2.reg(Qo_sa,ps,lagx=lag)
SLPregQs, intercept = st2.reg(Qs_sa,ps, lagx=lag)
SSTregQo, intercept = st2.reg(Qo_sa,sst,lagx=lag)
SSTregQs, intercept = st2.reg(Qs_sa,sst, lagx=lag)

SLPregQs_o, b = st2.reg(Q_s_o_sa,ps,lagx=lag)
SLPregQs_r, b = st2.reg(Q_s_r_sa,ps,lagx=lag)
SSTregQs_o, b = st2.reg(Q_s_o_sa,sst,lagx=lag)
SSTregQs_r, b = st2.reg(Q_s_r_sa,sst,lagx=lag)
...
#SSTregQo = SSTregQof - SSTregQoi
#SSTregQs = SSTregQsf - SSTregQsi

QsregQo, intercept = st2.reg(Qo_sa, Q_s)

tendsst = tendsst*(2*dt)

SLPtendregQo, b = st2.reg(Qo_sa,tendps)
SLPtendregQs, b = st2.reg(Qs_sa, tendps)

SSTtendregQo, b = st2.reg(Qo_sa,tendsst)
SSTtendregQs, b = st2.reg(Qs_sa,tendsst)

SLPtendregQs_o, b = st2.reg(Q_s_o_sa,tendps)
SLPtendregQs_r, b = st2.reg(Q_s_r_sa, tendps)

SSTtendregQs_o, b = st2.reg(Q_s_o_sa,tendsst)
SSTtendregQs_r, b = st2.reg(Q_s_r_sa,tendsst)

#reg = np.column_stack((Qo_sa,Qs_sa))
#
#def xr_regression(y):
#    X = sm.add_constant(reg, prepend=True) # regressor matrix
#    mod = sm.OLS(y.values, X, missing = 'drop') 
#    #mod = sm.GLSAR(y.values, X, 1, missing = 'drop') # MLR analysis with AR2 modeling
#    res = mod.fit()
#
#    return xr.DataArray(res.params[1:])
#
#tendsst_stacked = tendsst.stack(allpoints = ['lat', 'lon']).squeeze()
#tendsst_stacked = tendsst_stacked.reset_coords(drop=True)
#
#tendsst_coefs = tendsst_stacked.groupby('allpoints').apply(xr_regression)
#tendsst_coefs = tendsst_coefs.unstack('allpoints')
#
#tendps_stacked = tendps.stack(allpoints = ['lat', 'lon']).squeeze()
#tendps_stacked = tendps_stacked.reset_coords(drop=True)
#
#tendps_coefs = tendps_stacked.groupby('allpoints').apply(xr_regression)
#tendps_coefs = tendps_coefs.unstack('allpoints')


#reflon=305
#reflat=38
#
#SLPtendregQo_l, b = st2.reg(Qr.sel(lon=reflon,lat=reflat),tendps)
#SLPtendregQs_l, b = st2.reg(Q_s.sel(lon=reflon,lat=reflat), tendps)
#
#SSTtendregQo_l, b = st2.reg(Qr.sel(lon=reflon,lat=reflat),tendsst)
#SSTtendregQs_l, b = st2.reg(Q_s.sel(lon=reflon,lat=reflat),tendsst)
...
ax1.set_ylim(ymin,ymax)
ax1.set_title('$C_o\frac{\partial T}{\partial t}$')
o1,=ax1.plot(tendsst_sa.time, (Cbar_sa*tendsst_sa),color='k')
ax2.set_ylabel('Heat Flux (W/m$^{2}$)')
ax2.set_title(r'${Q_s}$')
ax2.set_ylim(ymin,ymax)
o2,=ax2.plot(Qs_sa.time, Qs_sa, color='C0')
o3,=ax3.plot(Qo_sa.time, Qo_sa, color='C2')
#plt.plot(sst.time, sst.sel(lon=reflon,lat=reflat), label='T', color='C5')
ax3.set_xlabel('Time (year)')
ax3.set_ylabel('Heat Flux (W/m$^{2}$)')
ax3.set_title(r'${Q_o}$')
ax3.set_ylim(ymin,ymax)
#plt.figlegend([o1,o2,o3], labels, loc = 'upper right', ncol=5, labelspacing=0.)
plt.savefig(fout + '{:s}_heatbudget_spatialave_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()

f, (ax2, ax3) = plt.subplots(2, 1, figsize=(12,10),sharex=True)
ax2.set_ylabel('Heat Flux (W/m$^{2}$)')
ax2.set_title(r'$Q_{s,o}$')
ax2.set_ylim(ymin,ymax)
o2,=ax2.plot(Q_s_o_sa.time, Q_s_o_sa, color='C0')
o3,=ax3.plot(Q_s_r_sa.time, Q_s_r_sa, color='C2')
#plt.plot(sst.time, sst.sel(lon=reflon,lat=reflat), label='T', color='C5')
ax3.set_xlabel('Time (year)')
ax3.set_ylabel('Heat Flux (W/m$^{2}$)')
ax3.set_title(r'$Q_{s,r}$')
ax3.set_ylim(ymin,ymax)
#plt.figlegend([o1,o2,o3], labels, loc = 'upper right', ncol=5, labelspacing=0.)
plt.savefig(fout + '{:s}_heatbudget_Qsdecomp_spatialave_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()
...
m,ax=mapper(var_T, bnds=bnds, cbfrac=cbfrac,title=r'$\sigma^2_T$', units=r'K$^{2}$', cmap=sstcmap, vmin=0, vmax=sstvmax)
ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=3,transform=cart.crs.PlateCarree(), zorder=5))
...
m,ax=mapper(SSTtendregQo, bnds=bnds, ct=False, cbfrac=cbfrac,title=r'SST and SLP tendency regressed onto $Q_o$', units=r'K W m$^{-2}$', cmap=fieldcmap, vmin=fieldmin,vmax=fieldmax)
ctstep = np.abs(psmax-psmin)/20
ctlevels = np.arange(psmin,psmax+ctstep, ctstep)
ct=plt.contour(x, y, SLPtendregQo, colors='k', levels=ctlevels, linewidths=1,  transform=cart.crs.PlateCarree())
...
ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=3,transform=cart.crs.PlateCarree(), zorder=5))
#poly = Polygon([(x1,y1),(x2,y1),(x2,y1),(x2,y2)],facecolor='none',edgecolor='black',linewidth=3,zorder=100, transform=cart.crs.PlateCarree())
#ax.add_patch(poly)
plt.savefig(fout + '{:s}_SSTSLPTENDregQo_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()

plt.figure(figsize=(12,10))
m,ax=mapper(SSTtendregQs, bnds=bnds, ct=False, cbfrac=cbfrac,title=r'SST and SLP tendency regressed onto $Q_s$', units=r'K/(W m$^{-2}$)', cmap=fieldcmap, vmin=fieldmin,vmax=fieldmax)
ctstep = np.abs(psmax-psmin)/20
ctlevels = np.arange(psmin,psmax+ctstep, ctstep)
ct=plt.contour(x, y, SLPtendregQs, colors='k', levels=ctlevels, linewidths=1,  transform=cart.crs.PlateCarree())
if np.any(np.round(ct.levels, 5) == 0):
    ct.collections[np.where(np.round(ct.levels, 5) == 0)[0][0]].set_linewidth(0)
    #ax.clabel(ct, fontsize=9, inline=1, fmt='%1.1f')
    for line in ct.collections:
        if line.get_linestyle() != [(None, None)]:
            line.set_linestyle([(0, (8.0, 8.0))])
ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=3,transform=cart.crs.PlateCarree(), zorder=5))
plt.savefig(fout + '{:s}_SSTSLPTENDregQs_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()


plt.figure(figsize=(12,10))
m,ax=mapper(SSTregQo, bnds=bnds, ct=False, cbfrac=cbfrac,title=r'SST and SLP regressed onto $Q_o$ (Lag {:1.0f})'.format(lag), units=r'K/(W m$^{-2}$)', cmap=fieldcmap, vmin=fieldmin, vmax=fieldmax)
ctstep = np.abs(psmax-psmin)/20
ctlevels = np.arange(psmin,psmax+ctstep, ctstep)
ct=plt.contour(x, y, SLPregQo, colors='k', levels=ctlevels, linewidths=1,  transform=cart.crs.PlateCarree())
if np.any(np.round(ct.levels, 5) == 0):
    ct.collections[np.where(np.round(ct.levels, 5) == 0)[0][0]].set_linewidth(0)
    #ax.clabel(ct, fontsize=9, inline=1, fmt='%1.1f')
    for line in ct.collections:
...
#poly = Polygon([(x1,y1),(x2,y1),(x2,y1),(x2,y2)],facecolor='none',edgecolor='black',linewidth=3,zorder=100, transform=cart.crs.PlateCarree())
#ax.add_patch(poly)
plt.savefig(fout + '{:s}_SSTSLPregQo_lag{:1.0f}_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, lag, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()

plt.figure(figsize=(12,10))
m,ax=mapper(SSTregQs, bnds=bnds, ct=False, cbfrac=cbfrac,title=r'SST and SLP regressed onto $Q_s$ (Lag {:1.0f})'.format(lag), units=r'K/(W m$^{-2}$)', cmap=fieldcmap, vmin=fieldmin, vmax=fieldmax)
ctstep = np.abs(psmax-psmin)/20
ctlevels = np.arange(psmin,psmax+ctstep, ctstep)
ct=plt.contour(x, y, SLPregQs, colors='k', levels=ctlevels, linewidths=1,  transform=cart.crs.PlateCarree())
if np.any(np.round(ct.levels, 5) == 0):
    ct.collections[np.where(np.round(ct.levels, 5) == 0)[0][0]].set_linewidth(0)
    #ax.clabel(ct, fontsize=9, inline=1, fmt='%1.1f')
    for line in ct.collections:
        if line.get_linestyle() != [(None, None)]:
            line.set_linestyle([(0, (8.0, 8.0))])
ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=3,transform=cart.crs.PlateCarree(), zorder=5))
plt.savefig(fout + '{:s}_SSTSLPregQs_lag{:1.0f}_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, lag, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()


plt.figure(figsize=(8,6))
m,ax=mapper(QsregQo, bnds=bnds, ct=False, cbfrac=0.05,title=r'$-Q_s$ regressed onto $Q_o$', units=r'W m$^{-2}$/(W m$^{-2}$)', cmap=fieldcmap, vmin=-1.5,vmax=1.5)
ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=3,transform=cart.crs.PlateCarree(), zorder=5))
#poly = Polygon([(x1,y1),(x2,y1),(x2,y1),(x2,y2)],facecolor='none',edgecolor='black',linewidth=3,zorder=100, transform=cart.crs.PlateCarree())
#ax.add_patch(poly)
plt.savefig(fout + '{:s}_QsregQo_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()



# Regression maps for ocean dynamics component of surface heat fluxes

plt.figure(figsize=(12,10))
m,ax=mapper(SSTtendregQs_o, bnds=bnds, ct=False, cbfrac=cbfrac,title=r'SST and SLP tendency regressed onto $Q_{s,o}$', units=r'K W m$^{-2}$', cmap=fieldcmap, vmin=fieldmin,vmax=fieldmax)
ctstep = np.abs(psmax-psmin)/20
ctlevels = np.arange(psmin,psmax+ctstep, ctstep)
ct=plt.contour(x, y, SLPtendregQs_o, colors='k', levels=ctlevels, linewidths=1,  transform=cart.crs.PlateCarree())
if np.any(np.round(ct.levels, 5) == 0):
    ct.collections[np.where(np.round(ct.levels, 5) == 0)[0][0]].set_linewidth(0)
    #ax.clabel(ct, fontsize=9, inline=1, fmt='%1.1f')
    for line in ct.collections:
...
#poly = Polygon([(x1,y1),(x2,y1),(x2,y1),(x2,y2)],facecolor='none',edgecolor='black',linewidth=3,zorder=100, transform=cart.crs.PlateCarree())
#ax.add_patch(poly)
plt.savefig(fout + '{:s}_SSTSLPTENDregQs_o_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()

plt.figure(figsize=(12,10))
m,ax=mapper(SSTtendregQs_r, bnds=bnds, ct=False, cbfrac=cbfrac,title=r'SST and SLP tendency regressed onto $Q_{s,r}$', units=r'K/(W m$^{-2}$)', cmap=fieldcmap, vmin=fieldmin,vmax=fieldmax)
ctstep = np.abs(psmax-psmin)/20
ctlevels = np.arange(psmin,psmax+ctstep, ctstep)
ct=plt.contour(x, y, SLPtendregQs_r, colors='k', levels=ctlevels, linewidths=1,  transform=cart.crs.PlateCarree())
if np.any(np.round(ct.levels, 5) == 0):
    ct.collections[np.where(np.round(ct.levels, 5) == 0)[0][0]].set_linewidth(0)
    #ax.clabel(ct, fontsize=9, inline=1, fmt='%1.1f')
    for line in ct.collections:
        if line.get_linestyle() != [(None, None)]:
            line.set_linestyle([(0, (8.0, 8.0))])
ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=3,transform=cart.crs.PlateCarree(), zorder=5))
plt.savefig(fout + '{:s}_SSTSLPTENDregQs_r_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()
...
plt.figure(figsize=(12,10))
m,ax=mapper(SSTregQs_o, bnds=bnds, ct=False, cbfrac=cbfrac,title=r'SST and SLP regressed onto {:s} (Lag {:1.0f})'.format(Q_s_o_name,lag), units=r'K/(W m$^{-2}$)', cmap=fieldcmap, vmin=fieldmin, vmax=fieldmax)
ctstep = np.abs(psmax-psmin)/20
ctlevels = np.arange(psmin,psmax+ctstep, ctstep)
ct=plt.contour(x, y, SLPregQs_o, colors='k', levels=ctlevels, linewidths=1,  transform=cart.crs.PlateCarree())
if np.any(np.round(ct.levels, 5) == 0):
    ct.collections[np.where(np.round(ct.levels, 5) == 0)[0][0]].set_linewidth(0)
    #ax.clabel(ct, fontsize=9, inline=1, fmt='%1.1f')
    for line in ct.collections:
...
#poly = Polygon([(x1,y1),(x2,y1),(x2,y1),(x2,y2)],facecolor='none',edgecolor='black',linewidth=3,zorder=100, transform=cart.crs.PlateCarree())
#ax.add_patch(poly)
plt.savefig(fout + '{:s}_SSTSLPregQs_o_lag{:1.0f}_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, lag, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()

plt.figure(figsize=(12,10))
m,ax=mapper(SSTregQs_r, bnds=bnds, ct=False, cbfrac=cbfrac,title=r'SST and SLP regressed onto {:s} (Lag {:1.0f})'.format(Q_s_r_name, lag), units=r'K/(W m$^{-2}$)', cmap=fieldcmap, vmin=fieldmin, vmax=fieldmax)
ctstep = np.abs(psmax-psmin)/20
ctlevels = np.arange(psmin,psmax+ctstep, ctstep)
ct=plt.contour(x, y, SLPregQs_r, colors='k', levels=ctlevels, linewidths=1,  transform=cart.crs.PlateCarree())
if np.any(np.round(ct.levels, 5) == 0):
    ct.collections[np.where(np.round(ct.levels, 5) == 0)[0][0]].set_linewidth(0)
    #ax.clabel(ct, fontsize=9, inline=1, fmt='%1.1f')
    for line in ct.collections:
        if line.get_linestyle() != [(None, None)]:
            line.set_linestyle([(0, (8.0, 8.0))])
ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=3,transform=cart.crs.PlateCarree(), zorder=5))
plt.savefig(fout + '{:s}_SSTSLPregQs_r_lag{:1.0f}_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, lag, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()
