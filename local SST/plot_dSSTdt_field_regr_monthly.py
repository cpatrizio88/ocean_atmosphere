...
fout = '/Volumes/GoogleDrive/My Drive/PhD/figures/atmosphere-ocean/localSST_global/'
...
NAsst = spatial_ave(sstprime[:,NAminlati:NAmaxlati,NAminloni:NAmaxloni], NAlats)
...
if lterm:
    ax.plot(tyears[ci:-ci], NAsst_lt, label='{:1.0f}-month RM'.format(N_map-1))
...
if lterm:
    ax.plot(tyears[ci:-ci], NAfield_lt, label='{:1.0f}-month RM'.format(N_map-1))
    ax.legend()
#plt.plot(tyears[ci:-ci], AMO_st, label='short-term residual')
#ax.fill_between(tyears[ci:-ci], 0, AMO_smooth, where= AMO_smooth>0, color='red')
#ax.fill_between(tyears[ci:-ci], 0, AMO_smooth, where= AMO_smooth<0, color='blue')
...
#ax.legend()
ax.set_xlabel('time (years)')
plt.savefig(fout + '{:s}_{:s}{:s}_monthly_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, sstsave, fsave, latbounds[0], latbounds[1], str(detr)[0]))
plt.close()
