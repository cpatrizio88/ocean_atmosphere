...
for i in range(nlat):   
     ...
     #for j in range(nlon):
        ...
        if corr:
             ...
             psstd = scaler.fit_transform(vprime_g)
             if lterm:
        ...
        for l, lag in enumerate(lags):
                 ...
                 if lag > 0:
                    ...
                    ulagcoefs = np.diag(np.ma.cov(sststd[:-lag,:], ustd[lag:], rowvar=False)[:N,N:])
                    vlagcoefs = np.diag(np.ma.cov(sststd[:-lag,:], vstd[lag:], rowvar=False)[:N,N:])
                    pslagcoefs = np.diag(np.ma.cov(sststd[:-lag,:], psstd[lag:], rowvar=False)[:N,N:])
                    if lterm:
                        ...
                        u_ltlagcoefs = np.diag(np.ma.cov(sststd_lt[:-lag,:], ustd_lt[lag:], rowvar=False)[:N,N:])
                        u_stlagcoefs = np.diag(np.ma.cov(sststd_st[:-lag,:], ustd_st[lag:], rowvar=False)[:N,N:])
                        
                        v_ltlagcoefs = np.diag(np.ma.cov(sststd_lt[:-lag,:], vstd_lt[lag:], rowvar=False)[:N,N:])
                        v_stlagcoefs = np.diag(np.ma.cov(sststd_st[:-lag,:], vstd_st[lag:], rowvar=False)[:N,N:])
                        
                        ps_ltlagcoefs = np.diag(np.ma.cov(sststd_lt[:-lag,:], psstd_lt[lag:], rowvar=False)[:N,N:])
                        ps_stlagcoefs = np.diag(np.ma.cov(sststd_st[:-lag,:], psstd_st[lag:], rowvar=False)[:N,N:])
                ...
                #THF LEADS SST
                    ...
                    if lterm:
                        ...
                        ulagcoefs_lt = np.diag(np.ma.cov(sststd_lt[-lag:,:], ustd_lt[:lag], rowvar=False)[:N,N:])
                        ulagcoefs_st = np.diag(np.ma.cov(sststd_st[-lag:,:], ustd_st[:lag], rowvar=False)[:N,N:])
                        
                        vlagcoefs_lt = np.diag(np.ma.cov(sststd_lt[-lag:,:], vstd_lt[:lag], rowvar=False)[:N,N:])
                        vlagcoefs_st = np.diag(np.ma.cov(sststd_st[-lag:,:], vstd_st[:lag], rowvar=False)[:N,N:])
                        
                        pslagcoefs_lt = np.diag(np.ma.cov(sststd_lt[-lag:,:], psstd_lt[:lag], rowvar=False)[:N,N:])
                        pslagcoefs_st = np.diag(np.ma.cov(sststd_st[-lag:,:], psstd_st[:lag], rowvar=False)[:N,N:])
                    #lagcoefs = np.diag(np.matmul(sststd[-lag:,:].T, fieldstd[:lag]))/M
                    #lagcoefs_lt = np.diag(np.matmul(sststd_lt[-lag:,:].T, fieldstd_lt[:lag]))/M
                    #lagcoefs_st = np.diag(np.matmul(sststd_st[-lag:,:].T, fieldstd_st[:lag]))/M
                 else:
                    ...
                    ulagcoefs = np.diag(np.ma.cov(sststd, ustd, rowvar=False)[:N,N:])                    
                    vlagcoefs = np.diag(np.ma.cov(sststd, vstd, rowvar=False)[:N,N:])
                    pslagcoefs = np.diag(np.ma.cov(sststd, psstd, rowvar=False)[:N,N:])
                    if lterm:
                        ...
                        ulagcoefs_lt = np.diag(np.ma.cov(sststd_lt, ustd_lt, rowvar=False)[:N,N:])
                        ulagcoefs_st = np.diag(np.ma.cov(sststd_st, ustd_st, rowvar=False)[:N,N:])
                        
                        vlagcoefs_lt = np.diag(np.ma.cov(sststd_lt, vstd_lt, rowvar=False)[:N,N:])
                        vlagcoefs_st = np.diag(np.ma.cov(sststd_st, vstd_st, rowvar=False)[:N,N:])
                        
                        pslagcoefs_lt = np.diag(np.ma.cov(sststd_lt, psstd_lt, rowvar=False)[:N,N:])
                        pslagcoefs_st = np.diag(np.ma.cov(sststd_st, psstd_st, rowvar=False)[:N,N:])
                 ...
                 ulagcorrs[l,i,:] = ulagcoefs
                 vlagcorrs[l,i,:] = vlagcoefs
                 pslagcorrs[l,i,:] = pslagcoefs
                 if lterm:
                     fieldlagcorrs_lt[l,i,:] = lagcoefs_lt
                     fieldlagcorrs_st[l,i,:] = lagcoefs_st
                     
                     ulagcorrs_lt[l,i,:] = ulagcoefs_lt
                     ulagcorrs_st[l,i,:] = ulagcoefs_st
                     
                     vlagcorrs_lt[l,i,:] = vlagcoefs_lt
                     vlagcorrs_st[l,i,:] = vlagcoefs_st
#                 
                     pslagcorrs_lt[l,i,:] = pslagcoefs_lt
                     pslagcorrs_st[l,i,:] = pslagcoefs_st
...
#    if fsave == 'sst':
#        plt.title(r'autoregression of SST')
#    else:
...
#        cb.set_ticks(ticks)
#        cb.set_ticklabels(ticklbls)
#        plt.title(r'regression of long-term {:s} on SST ({:1.0f}-month RM)'.format(ftitle, N_map-1))
...
#        cb.set_ticks(ticks)
#        cb.set_ticklabels(ticklbls)
#        plt.title(r'regression of short-term {:s} on SST (residual)'.format(ftitle))
...
#        plt.close()
    ...
    for leadi in range(len(lags)):
        ...
        cb.set_ticks(ticks)
        cb.set_ticklabels(ticklbls)
        plt.title(r'{:s} and 10-m Winds ({:1.0f} month lag)'.format(ftitle, lags[leadi]))
        plt.savefig(fout + '{:s}_monthlylocalsst_uvSLP{:s}_lag{:1.0f}corr_map_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, fsave, lags[leadi], latbounds[0], latbounds[1], str(detr)[0]))
        plt.close()
        ...
        if lterm:
            ...
            plt.savefig(fout + '{:s}_monthlylocalsst_uvSLP{:s}_lag{:1.0f}ltcorr_map_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, fsave, lags[leadi], latbounds[0], latbounds[1], str(detr)[0]))
            plt.close()
            ...
            cb.set_ticks(ticks)
            cb.set_ticklabels(ticklbls)
            plt.title(r'Short-term {:s} and 10-m Winds ({:1.0f} month lag)'.format(ftitle, lags[leadi]))
            plt.savefig(fout + '{:s}_montlylocalsst_uvSLP{:s}_lag{:1.0f}stcorr_map_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, fsave, lags[leadi], latbounds[0], latbounds[1], str(detr)[0]))
            plt.close()
