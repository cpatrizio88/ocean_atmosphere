...
tendsst.month.values = thf.month.values
#Cbar.month.values = thf.month.values

#Qr = Cbar*tendsst - (-thf + Q_net_surf) - Q_ek

Qr = Cbar*tendsst.groupby('month') -(-thf + Q_net_surf)
...
#Qr = Qr.transpose('time','month','lat','lon')
...
cov_Qr = np.multiply(tendsst.groupby('month'),Qr.groupby('month'))
cov_Qr = cov_Qr.groupby('month').sum(dim='time')/nyears

cov_Qs = np.multiply(tendsst.groupby('month'),Q_s.groupby('month'))
cov_Qs = cov_Qs.groupby('month').sum(dim='time')/nyears
#cov_Qek = st2.cov(tendsst, Q_ek)
#cov_Qek_f = st2.cov(tendsst,Q_ek_f)


cov_Qsbar = cov_Qs.mean(dim='month')
cov_Qrbar = cov_Qr.mean(dim='month')
...
H_var = (Cbar*tendsst.groupby('month')).var(dim='time')

# Compute contributions to SST variance
T_var_Qr = fac*cov_Qr
T_var_Qs = fac*cov_Qs

T_var_Qr = T_var_Qr.mean('month')
T_var_Qs = T_var_Qs.mean('month')
...
m,ax=mapper(cov_Qsbar, log=False, bnds=bnds, title='', units=r'K W m$^{-2}$ s$^{-1}$', cbfrac=cbfrac, cmap=fieldcmap, vmin=-1e-5, vmax=1e-5)
if drawbox:
    ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1,linestyle='dashed',transform=cart.crs.PlateCarree()))
plt.savefig(fout + '{:s}_covQs_ {:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()

m,ax=mapper(cov_Qrbar, log=False,bnds=bnds, title='', units=r'K W m$^{-2}$ s$^{-1}$',  cbfrac=cbfrac, cmap=fieldcmap, vmin=-1e-5, vmax=1e-5)
if drawbox:
    ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1,linestyle='dashed',transform=cart.crs.PlateCarree()))
