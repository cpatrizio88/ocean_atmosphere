...
fh = xr.open_dataset(fin +  'CESM2.CMIP6-piControl.001.pop.h.HMXL.110001-120012.nc')
...
h = h.rename({'nlat': 'lat', 'nlon':'lon'})

theta = theta.rename({'nlat': 'lat', 'nlon':'lon'})
Qs = Qs.rename({'nlat': 'lat', 'nlon':'lon'})
...
if Qs.lon.max() <= 180:
   Qs = Qs.assign_coords(lon=(Qs.lon % 360)).roll(lon=((Qs.shape[2] // 2)-1))
