...
stats_Qr = xr.apply_ufunc(new_linregress, Qr, tendsst,
...
stats_Qs = xr.apply_ufunc(new_linregress, Q_s, tendsst,
                       input_core_dims=[['time'], ['time']],
                       output_core_dims=[["parameter"]],
                       vectorize=True,
                       dask="parallelized",
...
# slope_Qr, int_Qr, cor_Qr, p_Qr, stderr_Qr = linregress(tendsst.stack(z=('lat','lon')), Qr.stack(z=('lat','lon')))
# slope_Qs, int_Qs, cor_Qs, p_Qs, stderr_Qs  = linregress(tendsst.stack(z=('lat','lon')), Q_s.stack(z=('lat','lon')))
...
cov_Qr = cov_Qr.where(p_Qr < p_thresh)
cov_Qs = cov_Qs.where(p_Qs < p_thresh)
...
mask_Qs=T_var_Qs.where(p_Qs < p_thresh, 1000)
mask_Qs = mask_Qs.where(p_Qs > p_thresh)
...
m.pcolormesh(p_Qs.lon, p_Qs.lat, mask_Qs, transform=cart.crs.PlateCarree(), cmap='gray', alpha=0.1, zorder=0)
if drawbox:
    ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1,linestyle='dashed',transform=cart.crs.PlateCarree()))
plt.savefig(fout + '{:s}_TvarQs_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()

mask_Qr=T_var_Qr.where(p_Qr < p_thresh, 1000)
mask_Qr = mask_Qr.where(p_Qr > p_thresh)
...
m.pcolormesh(p_Qr.lon, p_Qr.lat, mask_Qr, transform=cart.crs.PlateCarree(), cmap='gray', alpha=0.1, zorder=0)
if drawbox:
    ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1,linestyle='dashed',transform=cart.crs.PlateCarree()))
