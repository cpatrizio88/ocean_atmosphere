...
#ORAS5
ftheta = xr.open_dataset('/Users/cpatrizio/data/ORAS5/oras5_theta_1979to2018.nc')
fsst = xr.open_dataset('/Users/cpatrizio/data/ORAS5/oras5_sst_1979to2018.nc')
fthf = xr.open_dataset('/Users/cpatrizio/data/ORAS5/oras5_thf_1979to2018.nc')
fh = xr.open_dataset('/Users/cpatrizio/data/ORAS5/oras5_mxldepth_1979to2018.nc')
fmask = xr.open_dataset('/Users/cpatrizio/data/ORAS5/tmask_r1x1.nc')


h = fh.somxl010
thf = fthf.sohefldo 
sst = fsst.sosstsst
theta = ftheta.votemper

thf = thf.rename({'time_counter':'time'})
h = h.rename({'time_counter':'time'})
sst = sst.rename({'time_counter':'time'})
theta = theta.rename({'time_counter':'time', 'deptht': 'k'})
...
#
#sst = theta.isel(k=0)
theta_mxl = theta.where(mxlpoints).mean(dim='k')
...
if dataname == 'ERA5' or dataname == 'ORAS5':
...
mapper(hmean, bnds=bnds, logscale=False, log=False, title='Climatological Mixed Layer Depth', cbfrac=cbfrac, units=r'm', cmap=logcmap, vmin=0, vmax=300)
plt.savefig(fout + '{:s}_hbar_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()

mapper(fac, logscale=False, log=True, bnds=bnds, title='$G$', cbfrac=cbfrac, units=r'', cmap=logcmap, vmin=1e4, vmax=1e6)
