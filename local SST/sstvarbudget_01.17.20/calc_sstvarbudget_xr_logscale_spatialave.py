...
#spatial smoothing (boxcar average)
thf = thf.rolling(lon=lon_ave, min_periods=2, center=True).mean()
thf = thf.rolling(lat=lat_ave, min_periods=2, center=True).mean()

Q_net_surf = Q_net_surf.rolling(lon=lon_ave, min_periods=2, center=True).mean()
Q_net_surf = Q_net_surf.rolling(lat=lat_ave, min_periods=2, center=True).mean()

sst = sst.rolling(lon=lon_ave, min_periods=2, center=True).mean()
sst = sst.rolling(lat=lat_ave, min_periods=2, center=True).mean()

# tendsst = tendsst.rolling(lon=lon_ave, min_periods=2, center=True).mean()
# tendsst = tendsst.rolling(lat=lat_ave, min_periods=2, center=True).mean()
...
# Cbar = Cbar.rolling(lon=5, min_periods=2, center=True).mean()
# Cbar = Cbar.rolling(lat=5, min_periods=2, center=True).mean()
...
month = ps.groupby('time.month').apply(lambda x: x).month

winter = (month <= 4) | (month >= 10)

ps_var_winter = ps.where(winter).var(dim='time')

ps_var = ps.var(dim='time')
...
if fsave:
    ...
    Qr_save.to_netcdf(fin + '{:s}_Qoanom_monthly1981to2015.nc'.format(dataname))
...
# fieldcmap = plt.cm.get_cmap("RdBu_r",lut=20)

# plt.cm.register_cmap(name='CET_L17', cmap=sstcmap)

# sstcmap = plt.cm.get_cmap("CET_L17",lut=20)
#varmin=10**2
#varmax=10**4
...
mapper = Mapper()
mapper(ps_var, log=False, logscale=False, bnds=bnds, title=r'$p_{s}^2$', units=r'hPa$^{2}$', cbfrac=cbfrac, cmap=sstcmap, vmin=0, vmax=60)
plt.savefig(fout + '{:s}_psvar_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()

mapper = Mapper()
mapper(ps_var_winter, log=False, logscale=False, bnds=bnds, title=r'$p_{s}^2$', units=r'hPa$^{2}$', cbfrac=cbfrac, cmap=sstcmap, vmin=0, vmax=60)
plt.savefig(fout + '{:s}_psvar_winter_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()
...
#logcmap = plt.cm.get_cmap('Spectral_r', lut=20)
...
mapper(T_var_Qs/T_var_sum - T_var_Qr/T_var_sum, logscale=False, bnds=bnds, title='$Q_o$', cbfrac=cbfrac, units=r'', cmap=logcmap, vmin=fracvmin, vmax=fracvmax)
