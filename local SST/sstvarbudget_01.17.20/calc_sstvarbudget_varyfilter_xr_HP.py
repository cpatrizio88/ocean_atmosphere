...
for jj, Tn in enumerate(Tns):
    ...
    if Tn > 0:
        sst = st.butter_highpass_filter_xr(sst_raw, cutoff, fs, order)
        thf = st.butter_highpass_filter_xr(thf_raw, cutoff, fs, order)
        Q_net_surf = st.butter_highpass_filter_xr(Q_net_surf_raw, cutoff, fs, order)
        #Q_ek = st.butter_lowpass_filter_xr(Q_ek_raw, cutoff, fs, order)
        sst_ECCO = st.butter_highpass_filter_xr(sst_ECCO, cutoff, fs, order)
        Tmxl_ECCO = st.butter_highpass_filter_xr(Tmxl_ECCO, cutoff, fs, order)
...
#plt.legend()
fig.savefig(fout + '{:s}_sstvarbudget_varytimefilterHP_{:2.0f}Nto{:2.0f}N.png'.format(dataname, latbounds[0], latbounds[1]))
plt.close(fig)
