...
#mapper = Mapper()
#mapper(T_var, logscale=False, bnds=bnds, title=r'$\sigma_T^2$', units=r'K$^{2}$', cbfrac=cbfrac, cmap=sstcmap, vmin=0, vmax=sstvmax)
...
mapper = Mapper()
mapper(covQsQo, bnds=bnds, log=False, norm=lognorm, title=r'$Q_s, Q_o$ Covariance', units=r'W$^2$ m$^{-4}$', cbfrac=cbfrac, cmap=sstcmap, vmin=-varmax, vmax=-varmin)
...
mapper = Mapper()
m,ax=mapper(T_var, logscale=False, bnds=bnds, title=r'$\sigma_T^2$', units=r'K', cbfrac=cbfrac, cmap=sstcmap, vmin=0, vmax=sstvmax)
if drawbox:
...
m,ax=mapper(T_var_sum, bnds=bnds, title='SUM', units=r'K$^{2}$', cbfrac=cbfrac, cmap=sstcmap, vmin=0, vmax=sstvmax)
if drawbox:
    ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1,linestyle='dashed',transform=cart.crs.PlateCarree()))
...
m,ax=mapper(T_var_Qs, logscale=False, bnds=bnds, title='$Q_s$', units=r'K', cbfrac=cbfrac, cmap=fieldcmap, vmin=vmin, vmax=vmax)
if drawbox:
    ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1,linestyle='dashed',transform=cart.crs.PlateCarree()))
plt.savefig(fout + '{:s}_TvarQs_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()

m,ax=mapper(T_var_Qr, logscale=False, bnds=bnds, title='$Q_o$', units=r'K',  cbfrac=cbfrac, cmap=fieldcmap, vmin=vmin, vmax=vmax)
if drawbox:
    ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1,linestyle='dashed',transform=cart.crs.PlateCarree()))
...
#
#m,ax=mapper(T_var_Qr_error, bnds=bnds, title='$Q_o$', units=r'K$^{2}$',  cbfrac=cbfrac, cmap=fieldcmap, vmin=vmin, vmax=vmax)
#if drawbox:
#    ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1,linestyle='dashed',transform=cart.crs.PlateCarree()))
...
mapper(T_var_Qek, bnds=bnds, title='$Q_{ek}$', cbfrac=cbfrac, units=r'K$^{2}$', cmap=fieldcmap, vmin=vmin, vmax=vmax)
...
##
mapper(sst_mean, bnds=bnds, title='$Q_{ek}$', units=r'W/m$^{2}$', cmap=sstcmap, vmin=-10, vmax=30)
...
mapper(logfrac, bnds=bnds, title='$log(\sigma^2_{T,Q_s} / \sigma^2_{T,Q_o})$', cbfrac=cbfrac, units=r'', cmap=logcmap, vmin=logfracmin, vmax=logfracmax)
...
mapper(Tmxlfrac, bnds=bnds, title='$\sigma^2_{T,mxl} / \sigma^2_{T}$', cbfrac=cbfrac, units=r'', cmap=sstcmap, vmin=0.3, vmax=1.1)
...
#mapper(Tmxl_var/sst_var, bnds=bnds, title='$\sigma^2_{T,mxl} / \sigma^2_{T}$', cbfrac=cbfrac, units=r'', cmap=sstcmap, vmin=0.3, vmax=1.1)
...
mapper(Qr_var, bnds=bnds, log=True, title='$Q_o$ Variance', cbfrac=cbfrac, units=r'W$^{2}$ m$^{-4}$', cmap=sstcmap, vmin=10**2, vmax=10**4)
