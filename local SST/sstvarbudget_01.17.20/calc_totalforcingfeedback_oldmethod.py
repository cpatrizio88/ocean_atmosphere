...
# freqs_csd, r2cov = sig.csd(sst[:-2,:],sst[2:,:],scaling='spectrum',axis=0,detrend=False,nfft=nt)

# r2cov = r2cov[1:]
# r2cov = np.abs(r2cov)**2
# r2cov = r2cov/nt
...
if highpass:
    ...
    Qtotstarpower = Qtotstarpower[cutoffi:,:]
    Qsstarpower = Qsstarpower[cutoffi:,:]
    Qrstarpower = Qrstarpower[cutoffi:,:]
elif lowpass:
    cutoffi = np.where(f <= cutoff)[0][-1]
    f = f[lowmaxi:cutoffi]
    Tpower = Tpower[lowmaxi:cutoffi,:]
    Qtotpower = Qtotpower[lowmaxi:cutoffi,:]
    Qspower = Qspower[lowmaxi:cutoffi,:]
    Qrpower = Qrpower[lowmaxi:cutoffi,:]
    Qtotstarpower = Qtotstarpower[lowmaxi:cutoffi,:]
    Qsstarpower = Qsstarpower[lowmaxi:cutoffi,:]
    Qrstarpower = Qrstarpower[lowmaxi:cutoffi,:]
else:
...
fac1 = (dt**2)/(((2  - 2*np.cos(2*np.pi*f))*Cbar**2))

fac2 = 1/(2*np.pi*Cbar*freq_sec)**2

fac3 = (dt**2)/((Cbar**2*np.sin((2*np.pi*f))))
...
#Tvar = T_var
#T_var = Tpower.sum('freq')
Qtot_var_Tpow = Qtotpower.sum('freq')
Qs_var_Tpow = Qspower.sum('freq')
Qr_var_Tpow = Qrpower.sum('freq')
QsQr_cov_Tpow = QsQrpower.sum('freq')

Qtotstar_var_Tpow = Qtotstarpower.sum('freq')
Qsstar_var_Tpow = Qsstarpower.sum('freq')
Qrstar_var_Tpow = Qrstarpower.sum('freq')
QsQrstar_cov_Tpow = QsQrstarpower.sum('freq')
...
ave_Tvar_sum = st2.spatial_ave_xr(Tvar_sum,lats)
ave_Tvar_Qs = st2.spatial_ave_xr(Tvar_Qs,lats)
ave_Tvar_Qr = st2.spatial_ave_xr(Tvar_Qr,lats)
...
Tvar_lambdaQtot = Tvar_Qtot - Tvar_Qtotstar_nf
Tvar_lambdaQs = Tvar_Qs - Tvar_Qsstar_nf
Tvar_lambdaQr = Tvar_Qr - Tvar_Qrstar_nf

covQsstarT = st2.cov(Q_sstar,sst)
covQrstarT = st2.cov(Q_rstar,sst)

Tvar_lambdaQs_test = G*(lambdaQ_s**2 + lambdaQ_r*lambdaQ_s)*T_var + G*(2*lambdaQ_s + lambdaQ_r)*covQsstarT + G*lambdaQ_s*covQrstarT - (Tvar_Qsstar - Tvar_Qsstar_nf)

#Tvar_lambdaQs_test = G*(lambdaQ_s**2 + lambdaQ_r*lambdaQ_s)*T_var + G*(2*lambdaQ_s + lambdaQ_r)*covQsstarT + G*lambdaQ_s*covQrstarT 

#G*lambdaQ_s*covQrstarT
#G*(2*lambdaQ_s +lambdaQ_r)*covQsstarT 
...
ax[4].plot(f,(Gf/fac).sel(lat=latp, lon=lonp))
...
ax[0].set_xscale('log')
ax[0].set_xlim(0.006,0.5)
ax[0].plot(f,Qtotstarpower.sel(lat=latp,lon=lonp))
ax[0].set_title('$|Q^{*}_{tot}|^2$')
ax[0].axhline(0,color='k', linewidth=1)
ax[1].plot(f,Qsstarpower.sel(lat=latp,lon=lonp))
ax[1].set_title('$|Q^{*}_{s}|^2$')
ax[1].axhline(0,color='k', linewidth=1)
ax[2].set_title('$|Q^{*}_{o}|^2$')
ax[2].plot(f,Qrstarpower.sel(lat=latp,lon=lonp))
ax[2].axhline(0,color='k', linewidth=1)
ax[3].set_title('$|Q^{*}_{s} Q^{*}_{o}|$')
ax[3].plot(f,QsQrstarpower.sel(lat=latp,lon=lonp))
ax[3].axhline(0,color='k', linewidth=1)
ax[4].plot(f,(Qrstarpower + QsQrstarpower).sel(lat=latp,lon=lonp))
...
ax[4].set_ylim(-20,50)
ax[4].set_xlabel(r'Frequency (month$^{-1}$)')
plt.savefig(fout + 'QsQr_powerspectra.pdf')
...
# if drawbox:
#     ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1, linestyle='dashed', transform=cart.crs.PlateCarree()))
# plt.savefig(fout + '{:s}_Tpow_Qtot_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
# plt.close()
...
# if drawbox:
#     ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1, linestyle='dashed', transform=cart.crs.PlateCarree()))
# plt.savefig(fout + '{:s}_Tpow_sum_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
# plt.close()
...
# if drawbox:
#     ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1,linestyle='dashed',transform=cart.crs.PlateCarree()))
# plt.savefig(fout + '{:s}_Tpow_Qs_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
# plt.close()
...
# if drawbox:
#     ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1,linestyle='dashed',transform=cart.crs.PlateCarree()))
# plt.savefig(fout + '{:s}_Tpow_Qo_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
# plt.close()
...
# if drawbox:
#     ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1,linestyle='dashed',transform=cart.crs.PlateCarree()))
# plt.savefig(fout + '{:s}_Tpow_QsQr_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
# plt.close()
...
mapper = Mapper()
mapper(Qtotstar_var, bnds=bnds, logscale=False, log=True,  title=r'$\overline{Q^{\prime 2}_{tot}}$', units=r'W$^2$ m$^{-4}$', cbfrac=cbfrac, cmap=sstcmap, vmin=varmin, vmax=varmax)
...
mapper = Mapper()
mapper(covQsQrstar, logscale=False, bnds=bnds, log=False, norm=lognorm, title=r'$\overline{Q^*_sQ^*_o}$', units=r'W$^2$ m$^{-4}$', cbfrac=cbfrac, cmap=cc.cm.CET_L17_r, vmin=lvmin, vmax=lvmax)
...
if drawbox:
    ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1, linestyle='dashed', transform=cart.crs.PlateCarree()))
plt.savefig(fout + '{:s}_Tvar_Qtot_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()
...
if drawbox:
    ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1,linestyle='dashed',transform=cart.crs.PlateCarree()))
plt.savefig(fout + '{:s}_Tvar_QsQr_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()
...
mapper = Mapper()
m,ax=mapper(Tvar_Qtotstar, bnds=bnds, log=True, title=r'$\sigma_T^2$', units=r'K$^{2}$', cbfrac=cbfrac, cmap=sstcmap, vmin=0.01, vmax=sstvmax)
if drawbox:
    ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1, linestyle='dashed', transform=cart.crs.PlateCarree()))
...
m,ax=mapper(Tvar_Qsstar, norm=lognorm, bnds=bnds, title='$\widetilde{Q}^{*}_s$', units=r'K$^{2}$',  cbfrac=cbfrac, cmap=fieldcmap, vmin=-1.0, vmax=1.0)
if drawbox:
    ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1,linestyle='dashed',transform=cart.crs.PlateCarree()))
...
m,ax=mapper(Tvar_Qrstar, norm=lognorm, bnds=bnds, title='$\widetilde{Q}^{*}_o$', units=r'K$^{2}$',  cbfrac=cbfrac, cmap=fieldcmap, vmin=-1.0, vmax=1.0)
if drawbox:
    ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1,linestyle='dashed',transform=cart.crs.PlateCarree()))
...
m,ax=mapper(Tvar_QsQrstar, norm=lognorm, bnds=bnds, title='$\widetilde{Q}^{*}_s \widetilde{Q}^{*}_o$', units=r'K$^{2}$',  cbfrac=cbfrac, cmap=fieldcmap, vmin=vmin, vmax=vmax)
if drawbox:
    ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1,linestyle='dashed',transform=cart.crs.PlateCarree()))
...
m,ax=mapper(Tvar_sumstar, log=True, bnds=bnds, title='sum', units=r'K$^{2}$',  cbfrac=cbfrac, cmap=sstcmap, vmin=0.01, vmax=sstvmax)
if drawbox:
    ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1,linestyle='dashed',transform=cart.crs.PlateCarree()))
plt.savefig(fout + '{:s}_Tvar_sumstar_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()


#feedback contributions
m,ax=mapper(Tvar_lambdaQtot, norm=lognorm, bnds=bnds, title='$\widetilde{\lambda}_{tot}$', units=r'K$^{2}$',  cbfrac=cbfrac, cmap=fieldcmap, vmin=vmin, vmax=vmax)
if drawbox:
    ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1,linestyle='dashed',transform=cart.crs.PlateCarree()))
plt.savefig(fout + '{:s}_Tvar_lambdaQtot_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()

m,ax=mapper(Tvar_lambdaQs, norm=lognorm, bnds=bnds, title='$\widetilde{Q}_s - \widetilde{Q}^{*}_{s}$', units=r'K$^{2}$',  cbfrac=cbfrac, cmap=fieldcmap, vmin=vmin, vmax=vmax)
if drawbox:
    ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1,linestyle='dashed',transform=cart.crs.PlateCarree()))
plt.savefig(fout + '{:s}_Tvar_lambdaQs_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()

m,ax=mapper(Tvar_lambdaQr, norm=lognorm, bnds=bnds, title='$\widetilde{Q}_o - \widetilde{Q}^{*}_{o}$', units=r'K$^{2}$',  cbfrac=cbfrac, cmap=fieldcmap, vmin=vmin, vmax=vmax)
if drawbox:
    ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1,linestyle='dashed',transform=cart.crs.PlateCarree()))
plt.savefig(fout + '{:s}_Tvar_lambdaQo_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()

m,ax=mapper(Tvar_lambdaQs_test, norm=lognorm, bnds=bnds, title='$\widetilde{\lambda}_s$', units=r'K$^{2}$',  cbfrac=cbfrac, cmap=fieldcmap, vmin=vmin, vmax=vmax)
if drawbox:
    ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1,linestyle='dashed',transform=cart.crs.PlateCarree()))
plt.savefig(fout + '{:s}_Tvar_lambdaQs_test_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()

#forcing w/out feedback contributions
mapper = Mapper()
m,ax=mapper(Tvar_Qtotstar_nf, bnds=bnds, log=True, title=r'$\sigma_T^2$', units=r'K$^{2}$', cbfrac=cbfrac, cmap=sstcmap, vmin=0.01, vmax=sstvmax)
if drawbox:
    ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1, linestyle='dashed', transform=cart.crs.PlateCarree()))
plt.savefig(fout + '{:s}_Tvar_Qtotstar_nf_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()

m,ax=mapper(Tvar_Qsstar_nf, norm=lognorm, bnds=bnds, title='$\widetilde{Q}^{*}_{s,nf}$', units=r'K$^{2}$',  cbfrac=cbfrac, cmap=fieldcmap, vmin=vmin, vmax=vmax)
if drawbox:
    ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1,linestyle='dashed',transform=cart.crs.PlateCarree()))
plt.savefig(fout + '{:s}_Tvar_Qsstar_nf_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()

m,ax=mapper(Tvar_Qrstar_nf, norm=lognorm, bnds=bnds, title='$\widetilde{Q}^{*}_{o,nf}$', units=r'K$^{2}$',  cbfrac=cbfrac, cmap=fieldcmap, vmin=vmin, vmax=vmax)
if drawbox:
    ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1,linestyle='dashed',transform=cart.crs.PlateCarree()))
plt.savefig(fout + '{:s}_Tvar_Qostar_nf_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()

# m,ax=mapper(Tvar_QsQrstar_nf, norm=lognorm, bnds=bnds, title='$\widetilde{Q}^{*}_s \widetilde{Q}^{*}_o$', units=r'K$^{2}$',  cbfrac=cbfrac, cmap=fieldcmap, vmin=vmin, vmax=vmax)
# if drawbox:
#     ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1,linestyle='dashed',transform=cart.crs.PlateCarree()))
# plt.savefig(fout + '{:s}_Tvar_QsQrstar_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
# plt.close()

m,ax=mapper(Tvar_sumstar_nf, log=True, bnds=bnds, title='sum', units=r'K$^{2}$',  cbfrac=cbfrac, cmap=sstcmap, vmin=0.01, vmax=sstvmax)
if drawbox:
    ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1,linestyle='dashed',transform=cart.crs.PlateCarree()))
plt.savefig(fout + '{:s}_Tvar_sumstar_nf_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()



m,ax=mapper(Tvar_Qsstar_nf + Tvar_lambdaQs, norm=lognorm, bnds=bnds, title='$\widetilde{\lambda}_s + \widetilde{Q}^{*}_{s,nf}$', units=r'K$^{2}$',  cbfrac=cbfrac, cmap=fieldcmap, vmin=-1.0, vmax=1.0)
if drawbox:
    ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1,linestyle='dashed',transform=cart.crs.PlateCarree()))
plt.savefig(fout + '{:s}_Tvar_Qs_test_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()

m,ax=mapper(Tvar_Qrstar_nf + Tvar_lambdaQr, norm=lognorm, bnds=bnds, title='$\widetilde{\lambda}_o + \widetilde{Q}^{*}_{o,nf}$', units=r'K$^{2}$',  cbfrac=cbfrac, cmap=fieldcmap, vmin=-1.0, vmax=1.0)
if drawbox:
    ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1,linestyle='dashed',transform=cart.crs.PlateCarree()))
plt.savefig(fout + '{:s}_Tvar_Qr_test_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()


#error
m,ax=mapper(Tvar_sumstar - T_var, log=True, bnds=bnds, title='error', units=r'K$^{2}$',  cbfrac=cbfrac, cmap=sstcmap, vmin=0.01, vmax=sstvmax)
if drawbox:
    ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1,linestyle='dashed',transform=cart.crs.PlateCarree()))
plt.savefig(fout + '{:s}_Tvar_errorstar_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()
...
m,ax=mapper(Tvar_sumstar_nf + Tvar_lambdaQtot - T_var, log=True, bnds=bnds, title='error', units=r'K$^{2}$',  cbfrac=cbfrac, cmap=sstcmap, vmin=0.01, vmax=sstvmax)
if drawbox:
    ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1,linestyle='dashed',transform=cart.crs.PlateCarree()))
plt.savefig(fout + '{:s}_Tvar_error_feedbackcheck_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()
...
logfracabsstar = np.log(np.abs(Tvar_Qsstar)/np.abs(Tvar_Qrstar)) 
logfracabsstar_nf = np.log(np.abs(Tvar_Qsstar_nf)/np.abs(Tvar_Qrstar_nf)) 
...
mapper(logfracabsstar, logscale=False, bnds=bnds, title='$log(|\widetilde{Q}^{*}_s| / |\widetilde{Q}^{*}_o|)$', cbfrac=cbfrac, units=r'', cmap=logcmap, vmin=logfracmin, vmax=logfracmax)
plt.savefig(fout + '{:s}_TvarQsQoLOGFRACABSstar_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()

mapper(logfracabsstar_nf, logscale=False, bnds=bnds, title='$log(|\widetilde{Q}^{*}_{s,nf}| / |\widetilde{Q}^{*}_{o,nf}|)$', cbfrac=cbfrac, units=r'', cmap=logcmap, vmin=logfracmin, vmax=logfracmax)
plt.savefig(fout + '{:s}_TvarQsQoLOGFRACABSstar_nf_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()
...
mapper((Gstar-G)/G, logscale=False, log=False, bnds=bnds, title='$(G-G^{*}) / G$', cbfrac=cbfrac, units=r'', cmap=logcmap, vmin=-0.7, vmax=0.7)
plt.savefig(fout + '{:s}_Gdiff_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()
# mapper = Mapper()
