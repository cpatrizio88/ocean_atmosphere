...
# T_var_Qr = (1/Cbar)*cov_Qoint_T
# T_var_Qs = (1/Cbar)*cov_Qsint_T

Tsquare = sst**2
Tsquare_zero = (sst.isel(time=0))**2


# METHOD 1 CAN'T REPRODUCE VARIANCE. WHY?
QsT_int = (2/Cbar)*(Q_s*sst*dt).cumsum(dim='time')
QsT_int_bar = QsT_int.mean(dim='time')

QrT_int = (2/Cbar)*(Qr*sst*dt).cumsum(dim='time')
QrT_int_bar = QrT_int.mean(dim='time')

# T_var_Qs = QsT_int_bar
# T_var_Qr = QrT_int_bar

# METHOD 2 REPRODUCES LOW-FREQ CONTRIBUTIONS TO VARIANCE (due to time integration?)
Qs_int = (1/Cbar)*(Q_s*dt).cumsum(dim='time')
Qr_int = (1/Cbar)*(Qr*dt).cumsum(dim='time')

cov_Qs_int = st2.cov(sst,Qs_int)
cov_Qr_int = st2.cov(sst,Qr_int)
...
#T_var_sum = Tsquare_zero + T_var_Qr + T_var_Qs
...
Tsquare_plot = Tsquare.isel(lat=latp,lon=lonp)
T_plot = sst.isel(lat=latp,lon=lonp)

Tsquare_zero_plot = Tsquare_zero.isel(lat=latp,lon=lonp)

QsT_int_plot = QsT_int.isel(lat=latp,lon=lonp)
QrT_int_plot = QrT_int.isel(lat=latp,lon=lonp)

Qs_int_plot = Qs_int.isel(lat=latp,lon=lonp)
Qr_int_plot = Qr_int.isel(lat=latp,lon=lonp)

#Cbar_plot = Cbar.isel(lat=lat)


plt.plot(Tsquare_plot)
#plt.plot(QsT_int_plot, label='$Q_s$')
#plt.plot(QrT_int_plot, label='$Q_o$')
plt.plot(QsT_int_plot + QrT_int_plot, label='sum')
plt.legend()
plt.show()

print('Method 1 actual variance', Tsquare_plot.mean(dim='time'))
print('Method 1 estimated variance', (QsT_int_plot + QrT_int_plot).mean(dim='time'))

plt.plot(T_plot)
#plt.plot(QsT_int_plot, label='$Q_s$')
#plt.plot(QrT_int_plot, label='$Q_o$')
plt.plot(Qs_int_plot + Qr_int_plot, label='sum')
plt.legend()
plt.show()

print('Method 2 actual variance', T_plot.var(dim='time'))
print('Method 2 estimated variance', (Qs_int_plot + Qr_int_plot).var(dim='time'))
