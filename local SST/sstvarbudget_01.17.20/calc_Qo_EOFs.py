...
regridder = xe.Regridder(ps, ds_out, 'bilinear', reuse_weights=False)
ps  = regridder(ps)
regridder.clean_weight_file()
...
latts, lonns = np.meshgrid(lats, lons)

latts = latts.T
lonns = lonns.T

dlats = np.diff(latts,axis=0)
dlons = np.diff(lonns,axis=1)

dlats = (np.pi/180)*dlats
dlons = (np.pi/180)*dlons
...
if fsave:
    ...
    Qr_save.to_netcdf(fin + '{:s}_Qoanom_5eofs_monthly1981to2015.nc'.format(dataname))
...
fieldcmap = plt.cm.get_cmap("RdBu_r",lut=40)

plt.cm.register_cmap(name='CET_L17', cmap=sstcmap)

sstcmap = plt.cm.get_cmap("CET_L17",lut=40)
#varmin=10**2
#varmax=10**4
...
mapper = Mapper()
mapper(-sst_eof[0,:], logscale=False, bnds=bnds, log=False, title=r'SST EOF1', units=r'K', cbfrac=cbfrac, cmap=fieldcmap, vmin=-sstvmax, vmax=sstvmax)
plt.savefig(fout + '{:s}_sst_eof1_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()

mapper = Mapper()
mapper(sst_eof[1,:], logscale=False, bnds=bnds, log=False, title=r'SST EOF2', units=r'K', cbfrac=cbfrac, cmap=fieldcmap, vmin=-sstvmax, vmax=sstvmax)
plt.savefig(fout + '{:s}_sst_eof2_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()

mapper = Mapper()
mapper(sst_eof[2,:], logscale=False, bnds=bnds, log=False, title=r'SST EOF3', units=r'K', cbfrac=cbfrac, cmap=fieldcmap, vmin=-sstvmax, vmax=sstvmax)
plt.savefig(fout + '{:s}_sst_eof3_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()

mapper = Mapper()
mapper(Qs_eof[0,:], logscale=False, bnds=bnds, log=False, title=r'$Q_s$ EOF1', units=r'W/m$^2$', cbfrac=cbfrac, cmap=fieldcmap, vmin=-fieldmax, vmax=fieldmax)
plt.savefig(fout + '{:s}_Qs_eof_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()

mapper = Mapper()
mapper(Qo_eof[0,:], logscale=False, bnds=bnds, log=False, title=r'$Q_o$ EOF1', units=r'W/m$^2$', cbfrac=cbfrac, cmap=fieldcmap, vmin=-fieldmax, vmax=fieldmax)
plt.savefig(fout + '{:s}_Qo_eof_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()

mapper = Mapper()
mapper(Qo_eof[1,:], logscale=False, bnds=bnds, log=False, title=r'$Q_o$ EOF2', units=r'W/m$^2$', cbfrac=cbfrac, cmap=fieldcmap, vmin=-fieldmax, vmax=fieldmax)
plt.savefig(fout + '{:s}_Qo_eof2_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()

# mapper = Mapper()
# mapper(Qo_eof_projsst_map, logscale=False, bnds=bnds, log=False, title=r'SST projected onto EOF 1 of $Q_o$', units=r'K', cbfrac=cbfrac, cmap=fieldcmap, vmin=-sstvmax, vmax=sstvmax)
# plt.savefig(fout + '{:s}_SST_proj_Qoeof_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
# plt.close()

# mapper = Mapper()
# mapper(Qo_eof_projQo_map, logscale=False, bnds=bnds, log=False, title=r'$Q_o$ projected onto EOF 1 of $Q_o$', units=r'W/m$^2$', cbfrac=cbfrac, cmap=fieldcmap, vmin=-fieldmax, vmax=fieldmax)
# plt.savefig(fout + '{:s}_Qo_proj_Qoeof_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
# plt.close()

mapper = Mapper()
mapper(sst_reg_Qo_pc, logscale=False, bnds=bnds, log=False, title=r'SST regressed onto PC 1 of $Q_o$', units=r'K', cbfrac=cbfrac, cmap=fieldcmap, vmin=-sstvmax, vmax=sstvmax)
plt.savefig(fout + '{:s}_sst_reg_Qopc_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()

mapper = Mapper()
mapper(Qo_reg_Qo_pc, logscale=False, bnds=bnds, log=False, title=r'$Q_o$ regressed onto PC 1 of $Q_o$', units=r'W/m$^2$', cbfrac=cbfrac, cmap=fieldcmap, vmin=-fieldmax, vmax=fieldmax)
plt.savefig(fout + '{:s}_Qo_reg_Qopc_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()

mapper = Mapper()
mapper(sst_reg_Qs_pc, logscale=False, bnds=bnds, log=False, title=r'SST regressed onto PC 1 of $Q_s$', units=r'K', cbfrac=cbfrac, cmap=fieldcmap, vmin=-sstvmax, vmax=sstvmax)
plt.savefig(fout + '{:s}_sst_reg_Qspc_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()

mapper = Mapper()
mapper(Qs_reg_Qs_pc, logscale=False, bnds=bnds, log=False, title=r'$Q_s$ regressed onto PC 1 of $Q_s$', units=r'W/m$^2$', cbfrac=cbfrac, cmap=fieldcmap, vmin=-fieldmax, vmax=fieldmax)
plt.savefig(fout + '{:s}_Qs_reg_Qspc_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()
...
eof_num=np.arange(num_eofs)+1

plt.plot(eof_num, np.cumsum(sst_eof_varfracs[:num_eofs]), color='black', label='SST', linewidth=3)
plt.plot(eof_num, np.cumsum(Qs_eof_varfracs[:num_eofs]), color='green', label='$Q_s$', linewidth=3)
plt.plot(eof_num,np.cumsum(Qo_eof_varfracs[:num_eofs]), color='blue', label='$Q_o$', linewidth=3)

plt.xlim(1,num_eofs)
plt.xlabel('EOF No.')
plt.ylabel('Fraction of Variance (Cumulative)')
plt.gca().set_xticks(eof_num)
plt.gca().set_xticklabels(eof_num)
plt.ylim(0,1.0)
plt.legend(loc='lower right')
plt.savefig(fout + '{:s}_eof_varfracs__{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))

plt.close()

mapper = Mapper()
mapper(Qo_rec_var, bnds=bnds, logscale=False, log=True, title=r'$\overline{Q^{\prime 2}_{o,rec}}$',  units=r'W$^2$ m$^{-4}$', cbfrac=cbfrac, cmap=sstcmap, vmin=varmin, vmax=varmax)
plt.savefig(fout + '{:s}_Qrvar_rec_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
plt.close()

mapper = Mapper()
mapper(Qo_var, bnds=bnds, logscale=False, log=True, title=r'$\overline{Q^{\prime 2}_{o}}$',  units=r'W$^2$ m$^{-4}$', cbfrac=cbfrac, cmap=sstcmap, vmin=varmin, vmax=varmax)
...
# m,ax=mapper(T_var_Qs,  norm=lognorm, bnds=bnds,  title=r'$\overline{Q^{\prime 2}_s}$', units=r'K$^{2}$', cbfrac=cbfrac, cmap=fieldcmap, vmin=vmin, vmax=vmax)
# if drawbox:
#     ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1,linestyle='dashed',transform=cart.crs.PlateCarree()))
# plt.savefig(fout + '{:s}_TvarQs_{:1.0f}LP_{:2.0f}Nto{:2.0f}N_detr{:s}.png'.format(dataname, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
# plt.close()

# m,ax=mapper(T_var_Qr,  norm=lognorm, bnds=bnds,  title=r'$\overline{Q^{\prime 2}_o}$', units=r'K$^{2}$',  cbfrac=cbfrac, cmap=fieldcmap, vmin=vmin, vmax=vmax)
# if drawbox:
#     ax.add_patch(mpatches.Rectangle(xy=[x1,y1], width=(x2-x1), height=(y2-y1), facecolor='none', edgecolor='black',linewidth=1,linestyle='dashed',transform=cart.crs.PlateCarree()))
