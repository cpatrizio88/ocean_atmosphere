...
#subtract global annual mean to isolate local variability
...
for i in range(nlat):   
     ...
     #for j in range(nlon):
        ...
        #coefs = np.diag(np.matmul(sststd.T, fieldprime_g))/M
        ...
        coefs_lt = np.diag(np.ma.cov(sststd_lt, field_lt_g, rowvar=False)[:N,N:])
        coefs_st = np.diag(np.ma.cov(sststd_st, field_st_g, rowvar=False)[:N,N:])
...
#                 
                ...
                #THF LEADS SST
                 ...
                 fieldlagcorrs[lag+lagmax,i,:] = lagcoefs
                 fieldlagcorrs_lt[lag+lagmax,i,:] = lagcoefs_lt
                 fieldlagcorrs_st[lag+lagmax,i,:] = lagcoefs_st
#                 
...
plt.savefig(fout + '{:s}_localSST_{:s}_corr_map_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, fsave, latbounds[0], latbounds[1], str(detr)[0]))
plt.close()
...
if corr:
    if len(latboundar) > 1:
        plt.savefig(fout + '{:s}_localSST_{:s}_lagcorr_zonalavelocal_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, fsave, latbounds[0], latbounds[1], str(detr)[0]))
    else:
        plt.savefig(fout + '{:s}_localSST_{:s}_lagcorr_zonalave_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, fsave, latbounds[0], latbounds[1], str(detr)[0]))
else:
    if len(latboundar) > 1:
        plt.savefig(fout + '{:s}_localSST_{:s}_lagregr_zonalavelocal_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, fsave, latbounds[0], latbounds[1], str(detr)[0]))
    else:
        plt.savefig(fout + '{:s}_localSST_{:s}_lagregr_zonalave_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, fsave, latbounds[0], latbounds[1], str(detr)[0]))
plt.close() 
#
