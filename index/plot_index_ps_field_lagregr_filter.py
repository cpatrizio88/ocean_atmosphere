...
#matplotlib.rcParams.update({'contour.negative_linestyle': (5,10)}) 
maxlat = 90
minlat = -90
...
#                fieldclf_st = linear_model.LinearRegression()
    ...
    if plotmaps:
        ...
        for leadi in range(len(lags)):
                ...
                plt.title(r'{:s} and SLP ({:1.0f} month lag)'.format(ftitle, lags[leadi]))
                plt.savefig(fout + '{:s}_SST{:2.0f}Nto{:2.0f}N_{:s}SLP_lag{:1.0f}corr_map_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, lats[si], lats[ni], fsave, lags[leadi], latbounds[0], latbounds[1], str(detr)[0]))
                plt.close()
                ...
                if lterm:
                    ...
                    cb.set_ticks(ticks)
                    cb.set_ticklabels(ticklbls)
                    plt.title(r'Long-term ({:1.0f}-yr LP) {:s} and SLP ({:1.0f} month lag)'.format(Tn/12., ftitle, lags[leadi]))
                    plt.savefig(fout + '{:s}_SST{:2.0f}Nto{:2.0f}N_{:s}SLP_{:1.0f}LPlag{:1.0f}_map_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, lats[si], lats[ni], fsave, Tn/12., lags[leadi], latbounds[0], latbounds[1], str(detr)[0]))
                    plt.close()
                    ...
                    cb.set_ticks(ticks)
                    cb.set_ticklabels(ticklbls)
                    plt.title(r'Short-term ({:1.0f}-yr HP) and SLP {:s} ({:1.0f} month lag)'.format(Tn/12., ftitle, lags[leadi]))
                    plt.savefig(fout + '{:s}_SST{:2.0f}Nto{:2.0f}N_{:s}SLP_{:1.0f}HPlag{:1.0f}_map_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, lats[si], lats[ni], fsave, Tn/12., lags[leadi], latbounds[0], latbounds[1], str(detr)[0]))
                    plt.close()
    ...
    if lterm:
        ...
        #plt.suptitle('AMO ({:2.0f}$^{{\circ}}$N to {:2.0f}$^{{\circ}}$N)'.format(latbounds[0], latbounds[1]))
        ax = fig.add_subplot(131)
        h = ax.pcolor(lagg, latt, pslagcorrs_zonalave.T, vmin=fieldminlag, vmax=fieldmaxlag, cmap=cmap)
        ax.axvline(0, color='k')
        ax.set_title('Unfiltered {:s}'.format(ftitle))
        ...
        ax.set_xticklabels(laglabels)
        ax = fig.add_subplot(132)
        ax.pcolor(lagg, latt, pslagcorrs_lt_zonalave.T, vmin=fieldminlag, vmax=fieldmaxlag, cmap=cmap)
        ax.axvline(0, color='k')
        ...
        ax.set_xticklabels(laglabels)
        ax = fig.add_subplot(133)
        h = ax.pcolor(lagg, latt, pslagcorrs_st_zonalave.T, vmin=fieldminlag, vmax=fieldmaxlag, cmap=cmap)
        ax.axvline(0, color='k')
    ...
    if lterm:
        ...
        if corr:
            ...
            ax.set_ylabel('SLP ({:s})'.format(units))
        #if corr:
        #    ax.set_ylim((-0.5,0.5))
        ax.set_ylim((fieldminlag, fieldmaxlag))
        #ax.set_ylim(latbounds[0], latbounds[1])
