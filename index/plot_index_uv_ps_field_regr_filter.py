...
# apply the filter
field_mask = fieldprime.mask
ps_mask = psprime.mask
sst_mask = sstprime.mask
...
#fieldprime = np.ma.masked_array(fieldprime, mask=~np.isfinite(fieldprime))
#fieldprime = np.ma.masked_array(fieldprime, mask=np.abs(fieldprime) > 1e4)
#field_lt = np.ma.masked_array(field_lt, mask=field_mask)
#ps_lt = np.ma.masked_array(ps_lt, mask=ps_mask)
#sst_lt = np.ma.masked_array(sst_lt, mask=sst_mask)

print 'max field lt', np.ma.max(field_lt)
print 'max sst lt', np.ma.max(sst_lt)
print 'max ps lt', np.ma.max(ps_lt)
...
#            fieldcorrs_st[i,:] = np.squeeze(clf.coef_)
    ...
    if lterm:
        ...
        plt.savefig(fout + '{:s}_{:s}{:2.0f}Nto{:2.0f}N_sstSLPuv_{:1.0f}LPcorr_map_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, sstsave, lats[si], lats[ni], Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
        plt.close()
        ...
        plt.savefig(fout + '{:s}_{:s}{:2.0f}Nto{:2.0f}N_sstSLPuv_{:1.0f}HPcorr_map_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, sstsave, lats[si], lats[ni], Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
        plt.close()
    ...
    if plotwinds:
        ...
        ax.quiver(x[::uskip,::uskip], y[::uskip,::uskip], ucorrs[::uskip,::uskip], vcorrs[::uskip,::uskip], transform=cart.crs.PlateCarree(),  scale_units='inches', scale = 1, width=0.0015, headwidth=12, headlength=8, minshaft=2)
        ax.quiverkey(qv1, 0.90, -.13, 0.5, '0.5 m/s')
        if drawbox:
            poly = Polygon([(x1,y1),(x2,y2),(x3,y3),(x4,y4)],facecolor='none',edgecolor='black',linewidth=3,zorder=100)
            ax.add_patch(poly)
        ...
        plt.title(r'{:s} and 10-m Winds'.format(ftitle))
        ...
        if lterm:
            ...
            if drawbox:
                poly = Polygon([(x1,y1),(x2,y2),(x3,y3),(x4,y4)],facecolor='none',edgecolor='black',linewidth=3,zorder=100)
                ax.add_patch(poly)
            plt.title(r'Long-term {:s} and 10-m Winds ({:1.0f}-yr LP)'.format(ftitle, Tn/12.))
            plt.savefig(fout + '{:s}_{:s}{:2.0f}Nto{:2.0f}N_{:s}_{:1.0f}LPcorr_map_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, sstsave, lats[si], lats[ni], fsave, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
            plt.close()
            ...
            cb = plt.colorbar(plot, orientation = orient, label=r'{:s}'.format(units))
            qv1 = ax.quiver(x[::uskip,::uskip], y[::uskip,::uskip], ucorrs_st[::uskip,::uskip], vcorrs_st[::uskip,::uskip], transform=cart.crs.PlateCarree(), scale_units='inches', scale = 1, width=0.0015, headwidth=12, headlength=8, minshaft=2)
            ax.quiverkey(qv1, 0.90, -.13, 0.5, '0.5 m/s')
            if drawbox:
                poly = Polygon([(x1,y1),(x2,y2),(x3,y3),(x4,y4)],facecolor='none',edgecolor='black',linewidth=3,zorder=100)
                ax.add_patch(poly)
            cb.set_ticks(ticks)
            cb.set_ticklabels(ticklbls)
            plt.title(r'Short-term {:s} and 10-m Winds ({:1.0f}-yr HP)'.format(ftitle, Tn/12.))
            plt.savefig(fout + '{:s}_{:s}{:2.0f}Nto{:2.0f}N_{:s}_{:1.0f}HPcorr_map_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, sstsave, lats[si], lats[ni], fsave, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
            plt.close()
    ...
    if plotSLP:    
        ...
        if lterm:
            ...
            plt.savefig(fout + '{:s}_{:s}{:2.0f}Nto{:2.0f}N_{:s}SLP_{:1.0f}LPcorr_map_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, sstsave, lats[si], lats[ni], fsave, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
            plt.close()
            ...
            plt.savefig(fout + '{:s}_{:s}{:2.0f}Nto{:2.0f}N_{:s}SLP_{:1.0f}HPcorr_map_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, sstsave, lats[si], lats[ni], fsave, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
            plt.close()
            
        if plotwinds and plotSLP:
            plt.figure(figsize=(16,12))
            ax = plt.axes(projection=prj)
            ax.coastlines()
            ax.add_feature(cart.feature.LAND, zorder=99, edgecolor='k', facecolor='grey')
