...
Tns = np.array([4,8,12,18,24,36,48,60,72])

Tns = Tns*1.

avelagcorrs = np.zeros((len(Tns),len(lags)))

avelagcorrs_test = np.zeros((len(Tns),len(lags)))

avelagcorrs_test_s = np.zeros((len(Tns),len(lags)))
...
#        eloni = 165
        ...
        fieldindex = spatial_ave(fieldprime[:,si:ni,wi:ei], lats[si:ni])
        
        fieldindex_lt = butter_lowpass_filter(fieldindex, cutoff, fs, order)
        ...
        fieldindexstd = scaler.fit_transform(fieldindex.reshape(-1,1))
        fieldindexstd_lt = scaler.fit_transform(fieldindex_lt.reshape(-1,1))
        
        #avelagcorr_temp = np.correlate(indexstd, fieldindexstd, mode='full')
        if corr:
            avelagcorr_lt_temp = np.correlate(np.squeeze(indexstd_lt)/len(indexstd_lt), np.squeeze(fieldindexstd_lt), mode='full')
        else: 
            avelagcorr_lt_temp = np.correlate(np.squeeze(indexstd_lt)/len(indexstd_lt), np.squeeze(fieldindex_lt), mode='full')
        
        Nlags = len(avelagcorr_lt_temp)/2+1
        
        avelagcorrs_test[jj,:] = avelagcorr_lt_temp[Nlags-lagmax-lagstep:Nlags+lagmax+lagstep:lagstep]
        
#        #correlate indexstd with second index 
#        slati_s = 5    
#        nlati_s = 30
#        wloni_s = 305
#        eloni_s = 335
#        
#        
#        si_s = np.argmin(np.abs(lats - slati_s))
#        ni_s = np.argmin(np.abs(lats - nlati_s))
#        wi_s = np.argmin(np.abs(lons - wloni_s))
#        ei_s = np.argmin(np.abs(lons - eloni_s))
#        
#        fieldindex_s = spatial_ave(fieldprime[:,si_s:ni_s,wi_s:ei_s], lats[si_s:ni_s])
#        
#        fieldindex_lt_s = butter_lowpass_filter(fieldindex_s, cutoff, fs, order)
#        
#        fieldindexstd_s = scaler.fit_transform(fieldindex_s.reshape(-1,1))
#        fieldindexstd_lt_s = scaler.fit_transform(fieldindex_lt_s.reshape(-1,1))
#        
#         #avelagcorr_temp = np.correlate(indexstd, fieldindexstd, mode='full')
#        if corr:
#            avelagcorr_lt_temp = np.correlate(np.squeeze(indexstd_lt)/len(indexstd_lt), np.squeeze(fieldindexstd_lt_s), mode='full')
#        else: 
#            avelagcorr_lt_temp = np.correlate(np.squeeze(indexstd_lt)/len(indexstd_lt), np.squeeze(fieldindex_lt_s), mode='full')
#        
#        Nlags = len(avelagcorr_lt_temp)/2+1
#        
#        avelagcorrs_test_s[jj,:] = avelagcorr_lt_temp[Nlags-lagmax-lagstep:Nlags+lagmax+lagstep:lagstep]
#    
    ...
    #        cbstep = 2.5
        ...
        if plotmaps:
            ...
            for leadi in range(len(lags)):
                    ...
                    plt.savefig(fout + '{:s}_SST{:2.0f}Nto{:2.0f}N_{:s}_lag{:1.0f}_{:s}map_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, lats[si], lats[ni], fsave, lags[leadi], fcorrsave, latbounds[0], latbounds[1], str(detr)[0]))
                    plt.close()
                    ...
                    if lterm:
                        ...
                        cb.set_ticks(ticks)
                        cb.set_ticklabels(ticklbls)
                        plt.title(r'Long-term ({:1.1f}-yr LP) {:s} ({:1.0f} month lag)'.format(Tn/12., ftitle, lags[leadi]))
                        plt.savefig(fout + '{:s}_SST{:2.0f}Nto{:2.0f}N_{:s}_{:1.0f}LPlag{:1.0f}_{:s}map_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, lats[si], lats[ni], fsave, Tn, lags[leadi], fcorrsave, latbounds[0], latbounds[1], str(detr)[0]))
                        plt.close()
                        ...
                        cb.set_ticks(ticks)
                        cb.set_ticklabels(ticklbls)
                        plt.title(r'Short-term ({:1.1f}-yr HP) {:s} ({:1.0f} month lag)'.format(Tn/12., ftitle, lags[leadi]))
                        plt.savefig(fout + '{:s}_SST{:2.0f}Nto{:2.0f}N_{:s}_{:1.0f}HPlag{:1.0f}_{:s}map_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, lats[si], lats[ni], fsave, Tn, lags[leadi], fcorrsave, latbounds[0], latbounds[1], str(detr)[0]))
                        plt.close()
...
#        else:
        ...
        if lterm:
            ...
            ax.axvline(0, color='k')
            ax.set_title('Long-term {:s} ({:1.1f}-yr LP)'.format(ftitle, Tn/12.))
            ax.set_xlabel(r'{:s} lag ({:s})'.format(ftitle, lagunits))
            ax.set_ylim(latbounds[0], latbounds[1])
            ax.set_xticks(laglabels)
            ax.axhline(0, color='k', linewidth=1)
            ...
            ax.axvline(0, color='k')
            ax.set_title('Short-term {:s} ({:1.1f}-yr HP)'.format(ftitle, Tn/12.))
            ax.set_xlabel(r'{:s} lag ({:s})'.format(ftitle, lagunits))
            ax.set_ylim(latbounds[0], latbounds[1])
            ax.set_xticks(laglabels)
            ax.axhline(0, color='k', linewidth=1)
        ...
        avelagcorrs[jj,:] = NAfieldlagcorr_lt_ave 
        ...
        if lterm:
            ...
            ax.set_xticklabels(laglabels)
            if corr:
                    plt.savefig(fout + '{:s}_SST{:2.0f}Nto{:2.0f}N_{:s}_lagcorr_lagmax{:3.0f}_{:1.0f}LP_NAave_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, lats[si], lats[ni], fsave, lagmax, Tn, latbounds[0], latbounds[1], str(detr)[0]))
            else:
                    plt.savefig(fout + '{:s}_SST{:2.0f}Nto{:2.0f}N_{:s}_lagregr_lagmax{:3.0f}_{:1.0f}LP_NAave_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, lats[si], lats[ni], fsave, lagmax, Tn, latbounds[0], latbounds[1], str(detr)[0]))
            plt.close() 
...
avelagcorrs_new = np.vstack([avelagcorrs, NAfieldlagcorr_ave])
Tns_new = np. insert(Tns,0,0)

lagg, Tnn = np.meshgrid(lags-lagoffset, Tns_new)
#Plot average lagged correlation between local SST and field over a specific region, as a function of time smoothing
fig=plt.figure(figsize=(10,6))
#plt.suptitle('AMO ({:2.0f}$^{{\circ}}$N to {:2.0f}$^{{\circ}}$N)'.format(latbounds[0], latbounds[1]))
ax = plt.gca()
h = ax.pcolor(lagg, Tnn, avelagcorrs_new, vmin=fieldminlag, vmax=fieldmaxlag, cmap=cmap)
ax.axvline(0, color='k')
if corr:
    ax.set_title('{:s}'.format(ftitle))
else:
    ax.set_title('{:s}'.format(ftitle))
ax.set_xlabel(r'{:s} Lag ({:s})'.format(ftitle, lagunits))
ax.set_ylabel('Time Smoothing (months)')
#ax.set_ylim(latbounds[0], latbounds[1])
ax.axhline(0, color='k', linewidth=1)
ax.set_xticks(laglabels)
ax.set_xticklabels(laglabels)
...
ax.axvline(0, color='k')
if corr:
        plt.savefig(fout + '{:s}_SST{:2.0f}Nto{:2.0f}N_{:s}_lagcorr_lagmax{:3.0f}_varytimefilter_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, lats[si], lats[ni], fsave, lagmax, latbounds[0], latbounds[1], str(detr)[0]))
else:
        plt.savefig(fout + '{:s}_SST{:2.0f}Nto{:2.0f}N_{:s}_lagregr_lagmax{:3.0f}_varytimefilter_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, lats[si], lats[ni], fsave, lagmax, latbounds[0], latbounds[1], str(detr)[0]))
plt.close() 


 
##avelagcorr_temp = np.correlate(indexstd, fieldindexstd, mode='full')
#avelagcorr_temp = np.correlate(np.squeeze(indexstd)/len(indexstd), np.squeeze(fieldindexstd), mode='full')
#
#Nlags = len(avelagcorr_temp)/2+1
#
#avelagcorrs_temp = avelagcorr_temp[Nlags-lagmax-lagstep:Nlags+lagmax+lagstep:lagstep]
#
#avelagcorrs_new = np.vstack([avelagcorrs_test, avelagcorrs_temp])
#Tns_new = np. insert(Tns,0,0)
#
#
#
#lagg, Tnn = np.meshgrid(lags-lagoffset, Tns_new)
##Plot average lagged correlation between SST and field over a specific region, as a function of time smoothing
#fig=plt.figure(figsize=(10,6))
##plt.suptitle('AMO ({:2.0f}$^{{\circ}}$N to {:2.0f}$^{{\circ}}$N)'.format(latbounds[0], latbounds[1]))
#ax = plt.gca()
#h = ax.pcolor(lagg, Tnn, avelagcorrs_new[:,::-1], vmin=fieldminlag, vmax=fieldmaxlag, cmap=cmap)
#ax.axvline(0, color='k')
#if corr:
#    ax.set_title('{:s}'.format(ftitle))
#else:
...
#ax.axvline(0, color='k')
#if corr:
#        plt.savefig(fout + '{:s}_SST{:2.0f}Nto{:2.0f}N_AVERAGE{:s}_lagcorr_lagmax{:3.0f}_varytimefilter_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, lats[si], lats[ni], fsave, lagmax, latbounds[0], latbounds[1], str(detr)[0]))
#else:
#        plt.savefig(fout + '{:s}_SST{:2.0f}Nto{:2.0f}N_AVERAGE{:s}_lagregr_lagmax{:3.0f}_varytimefilter_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, lats[si], lats[ni], fsave, lagmax, latbounds[0], latbounds[1], str(detr)[0]))
#plt.close() 
#
#
##avelagcorr_temp = np.correlate(indexstd, fieldindexstd, mode='full')
#avelagcorr_temp_s = np.correlate(np.squeeze(indexstd)/len(indexstd), np.squeeze(fieldindexstd_s), mode='full')
#
##Nlags = len(avelagcorr_temp)/2+1
##
#avelagcorrs_temp_s = avelagcorr_temp[Nlags-lagmax-lagstep:Nlags+lagmax+lagstep:lagstep]
#
#avelagcorrs_new_s = np.vstack([avelagcorrs_test_s, avelagcorrs_temp_s])
##Tns_new = np. insert(Tns,0,0)
#
#
#
##lagg, Tnn = np.meshgrid(lags-lagoffset, Tns_new)
##Plot average lagged correlation between SST and field over second specific region, as a function of time smoothing
#fig=plt.figure(figsize=(10,6))
##plt.suptitle('AMO ({:2.0f}$^{{\circ}}$N to {:2.0f}$^{{\circ}}$N)'.format(latbounds[0], latbounds[1]))
#ax = plt.gca()
#h = ax.pcolor(lagg, Tnn, avelagcorrs_new_s[:,::-1], vmin=fieldminlag, vmax=fieldmaxlag, cmap=cmap)
#ax.axvline(0, color='k')
#if corr:
#    ax.set_title('{:s}'.format(ftitle))
#else:
...
#ax.axvline(0, color='k')
#if corr:
#        plt.savefig(fout + '{:s}_SST{:2.0f}Nto{:2.0f}N_AVERAGE{:s}{:2.0f}Nto{:2.0f}N_lagcorr_lagmax{:3.0f}_varytimefilter_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, lats[si], lats[ni], fsave, lats[si_s], lats[ni_s], lagmax, latbounds[0], latbounds[1], str(detr)[0]))
#else:
#        plt.savefig(fout + '{:s}_SST{:2.0f}Nto{:2.0f}N_AVERAGE{:s}{:2.0f}Nto{:2.0f}N_lagregr_lagmax{:3.0f}_varytimefilter_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, lats[si], lats[ni], fsave, lats[si_s], lats[ni_s],  lagmax, latbounds[0], latbounds[1], str(detr)[0]))
#plt.close()
#
