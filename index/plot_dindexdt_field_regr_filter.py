...
latw = 10  
slats = np.arange(0,50+latw,latw) 
...
#                sstcorrs_st[i,:] = np.squeeze(clf.coef_)
                ...
                sstcoefs_lt = np.diag(np.ma.cov(indexstd_ltrep, sst_lt_g, rowvar=False)[:N,N:])
                sstcoefs_st = np.diag(np.ma.cov(indexstd_strep, sst_st_g, rowvar=False)[:N,N:])
                
                fieldcoefs_lt = np.diag(np.ma.cov(indexstd_ltrep, field_lt_g, rowvar=False)[:N,N:])
                fieldcoefs_st = np.diag(np.ma.cov(indexstd_strep, field_st_g, rowvar=False)[:N,N:])
...
#        cbstep = 2.5
    ...
    #plt.figure()
    #ax=plt.gcf().gca()
    ax.plot(tyears[1:-1], index)
    ax.plot(tyears[1:-1], index_lt, label='{:1.0f}-yr LP'.format((Tn/12.)))
    ...
    ax.set_title(r'$\partial T_s / \partial t_{{{0}^\circ-{1}^\circ N}}$'.format(np.round(np.abs(lats[si]),0), np.round(lats[ni],0)))
    ax.axhline(0, color='black')
    ax.set_ylim(-0.85,0.85)
    ax.set_ylabel(r'$\partial T_s / \partial t$ (K/month)')
    ax.set_xlabel('time (years)')
    ax.legend(loc='upper right')
    ax = fig.add_subplot(212)
    #plt.figure()
    #ax=plt.gcf().gca()
    ax.plot(tyears[1:-1], NAfield)
    ax.plot(tyears[1:-1], NAfield_lt, label='{:1.0f}-yr LP'.format((Tn/12.)))
    ax.legend(loc='upper right')
    ...
    #ax.legend()
    ax.set_xlabel('time (years)')
    plt.savefig(fout + '{:s}_dSSTdt{:s}{:2.0f}Nto{:2.0f}N_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, fsave, lats[si], lats[ni], latbounds[0], latbounds[1], str(detr)[0]))
    plt.close()
    ...
    plt.title('Correlation between CTI and $\partial T_s / \partial t_{{{0}^\circ-{1}^\circ N}}$'.format(np.round(np.abs(lats[si]),0), np.round(lats[ni],0)))
    plt.ylabel('Correlation')
    plt.savefig(fout + '{:s}_CTI_dSSTdt{:2.0f}Nto{:2.0f}N_lagcorr_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, lats[si], lats[ni], latbounds[0], latbounds[1], str(detr)[0]))
    plt.close()
    ...
    plt.savefig(fout + '{:s}_dSSTdt{:2.0f}Nto{:2.0f}N_{:s}_corr_map_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, lats[si], lats[ni], fsave, latbounds[0], latbounds[1], str(detr)[0]))
    plt.close()
    ...
    if lterm:
        ...
        plt.savefig(fout + '{:s}_dSSTdt{:2.0f}Nto{:2.0f}N_{:s}_{:1.0f}LPcorr_map_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname, lats[si], lats[ni], fsave, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
        plt.close()
        ...
        plt.savefig(fout + '{:s}_dSSTdt{:2.0f}Nto{:2.0f}N_{:s}_{:1.0f}HPcorr_map_{:2.0f}Nto{:2.0f}N_detr{:s}.pdf'.format(dataname,  lats[si], lats[ni], fsave, Tn/12., latbounds[0], latbounds[1], str(detr)[0]))
        plt.close()
