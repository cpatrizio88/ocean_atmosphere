...
from ocean_atmosphere.misc_fns import an_ave, spatial_ave, running_mean, calc_NA_globeanom, calc_NA_globeanom3D, detrend, butter_lowpass_filter, regressout_x, cov2_coeff, corr2_coeff
...
#lats = field1.getLatitude()[:]
#lons = field1.getLongitude()[:]
...
#                            fieldlagcoefs_st = np.diag(np.ma.corrcoef(indexstd_strep[:-lag,...], field_st_g[lag:,...], rowvar=False)[:N,N:])
    ...
    fieldlagcorrs = fieldcorrs.reshape(len(lags), nplev, nlat, nlon)
    ...
    #Plot zonally-averaged fieldlagcorrs over 5 to 20 N and 45 to 60 N (lag, pressure)
